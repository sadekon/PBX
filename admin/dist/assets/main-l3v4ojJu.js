...
features:
  conversational_ai:
    ...
    `}var ae=async()=>{try{let e=(await(await fetch(`/api/framework/predictive-dialing/campaigns`,{headers:pbxAuthHeaders()})).json()).campaigns??[],t=document.getElementById(`campaigns-list`);if(e.length===0){t.innerHTML=`<p style="color: #666;">No campaigns created yet. Framework ready for campaign management.</p>`;return}t.innerHTML=`
            <table class="data-table">
                <thead>
                    <tr>
                        <th>Campaign Name</th>
    ...
    `}var d=async()=>{try{let e=(await(await fetch(`/api/framework/voice-biometrics/profiles`,{headers:pbxAuthHeaders()})).json()).profiles??[],t=document.getElementById(`biometric-profiles-list`);if(e.length===0){t.innerHTML=`<p style="color: #666;">No users enrolled yet. Framework ready for voice enrollment.</p>`;return}t.innerHTML=`
            <table class="data-table">
                <thead>
                    <tr>
                        <th>Extension</th>
...
POST /api/framework/bi-integration/test-connection</pre>
    ...
    `}var ve=async e=>{let t=document.getElementById(`bi-export-format`)?.value||`csv`,n=document.getElementById(`bi-date-range`)?.value||`last7days`,r=event.target,i=r.textContent;r.textContent=`Exporting...`,r.disabled=!0;try{let r=await(await fetch(`/api/framework/bi-integration/export`,{method:`POST`,headers:pbxAuthHeaders(),body:JSON.stringify({dataset:e,format:t,date_range:n})})).json();r.success?alert(`✅ Export successful!\n\nDataset: ${r.dataset}\nFormat: ${r.format}\nFile: ${r.file_path}\n\nThe export has been created on the server.`):alert(`❌ Export failed: ${r.error}`)}catch(e){alert(`❌ Export error: ${e.message}`)}finally{r.textContent=i,r.disabled=!1}};function ye(){return(async()=>{try{let e=await(await fetch(`/api/framework/call-tagging/statistics`,{headers:pbxAuthHeaders()})).json();document.getElementById(`tagging-stats-display`).innerHTML=`
            ...
            `}catch(e){console.error(`Error loading tagging statistics:`,e)}})(),setTimeout(()=>{f(),p()},100),`
...
POST /api/framework/call-tagging/classify/{call_id}</pre>
    ...
    `}var f=async()=>{try{let e=(await(await fetch(`/api/framework/call-tagging/tags`,{headers:pbxAuthHeaders()})).json()).tags??[],t=document.getElementById(`tags-list`);if(e.length===0){t.innerHTML=`<p style="color: #666;">No tags created yet. Click "+ Create Tag" to add your first tag.</p>`;return}t.innerHTML=`
    ...
    `),document.getElementById(`create-tag-form`).onsubmit=async function(e){e.preventDefault();let t=new FormData(e.target),n={name:t.get(`name`),description:t.get(`description`)||``,color:t.get(`color`)||`#007bff`};try{let e=await(await fetch(`/api/framework/call-tagging/tag`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify(n)})).json();e.success?(alert(`Tag created successfully!`),xe(),f(),be()):alert(`Error creating tag: ${e.error??`Unknown error`}`)}catch(e){alert(`Error creating tag: `+e.message)}}}function Ce(){let e=document.getElementById(`create-rule-dialog`);e&&e.remove()}function we(){Ce(),document.body.insertAdjacentHTML(`beforeend`,`
    ...
    `),document.getElementById(`create-rule-form`).onsubmit=async function(e){e.preventDefault();let t=new FormData(e.target),n={name:t.get(`name`),tag_id:t.get(`tag_id`),priority:parseInt(t.get(`priority`))||100,conditions:[{type:t.get(`condition_type`),value:t.get(`condition_value`)}]};try{let e=await(await fetch(`/api/framework/call-tagging/rule`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify(n)})).json();e.success?(alert(`Rule created successfully!`),Ce(),p(),be()):alert(`Error creating rule: ${e.error??`Unknown error`}`)}catch(e){alert(`Error creating rule: `+e.message)}}}var Te=async e=>{if(confirm(`Are you sure you want to delete this tag?`))try{await fetch(`/api/framework/call-tagging/tags/${e}`,{method:`DELETE`,headers:pbxAuthHeaders()}),await f()}catch(e){alert(`Error: ${e.message}`)}},Ee=async e=>{if(confirm(`Are you sure you want to delete this rule?`))try{await fetch(`/api/framework/call-tagging/rules/${e}`,{method:`DELETE`,headers:pbxAuthHeaders()}),await p()}catch(e){alert(`Error: ${e.message}`)}},De=async e=>{try{await fetch(`/api/framework/call-tagging/rules/${e}/toggle`,{method:`POST`,headers:pbxAuthHeaders()}),await p()}catch(e){alert(`Error: ${e.message}`)}};function Oe(){return`
...
features:
  team_messaging:
    ...
    `}window.frameworkFeatures={loadFrameworkOverview:a,loadClickToDialTab:o,loadVideoConferencingTab:l,loadConversationalAITab:te,loadPredictiveDialingTab:ie,loadVoiceBiometricsTab:ue,loadCallQualityPredictionTab:he,loadVideoCodecTab:ge,loadBIIntegrationTab:_e,loadCallTaggingTab:ye,loadMobileAppsTab:Oe,loadMobileNumberPortabilityTab:je,loadRecordingAnalyticsTab:Me,loadCallBlendingTab:Ne,loadVoicemailDropTab:Pe,loadGeographicRedundancyTab:Fe,loadDNSSRVFailoverTab:Ie,loadSessionBorderControllerTab:Le,loadDataResidencyTab:Re,loadTeamMessagingTab:ze,viewClickToDialHistory:c,loadConversationalAIStats:ne,loadMobileAppsStats:ke,exportBIDataset:ve,loadCallTags:f,loadTaggingRules:p,loadTagStatistics:be,showCreateTagDialog:Se,showCreateRuleDialog:we,deleteTag:Te,deleteRule:Ee,toggleRule:De,loadPredictiveDialingCampaigns:ae,loadPredictiveDialingStats:oe,showCreateCampaignDialog:ce,toggleCampaign:le,loadVoiceBiometricProfiles:d,loadVoiceBiometricStats:de,showEnrollUserDialog:pe,hideEnrollUserDialog:fe,deleteVoiceProfile:me,hideCreateCampaignDialog:se,hideCreateTagDialog:xe,hideCreateRuleDialog:Ce,submitConversationalAIConfig:re,submitMobileAppsConfig:Ae};for(let e of Object.keys(window.frameworkFeatures))typeof window.frameworkFeatures[e]==`function`&&(window[e]=window.frameworkFeatures[e]);var Be=`80`,Ve=`443`,He=`9000`,Ue=3e4;function m(){let e=document.querySelector(`meta[name="api-base-url"]`);return e&&e.content?e.content:window.location.port===He||window.location.port===``||window.location.port===Be||window.location.port===Ve?window.location.origin:`${window.location.protocol}//${window.location.hostname||`localhost`}:${He}`}function h(){let e=localStorage.getItem(`pbx_token`),t={"Content-Type":`application/json`};return e&&(t.Authorization=`Bearer ${e}`),t}async function g(e,t={},n=Ue){if(t.signal)throw Error(`fetchWithTimeout does not support custom abort signals. Use the timeout parameter instead.`);let r=new AbortController,i=setTimeout(()=>r.abort(),n);try{return await fetch(e,{...t,signal:r.signal})}catch(e){throw e instanceof Error&&e.name===`AbortError`?Error(`Request timed out`):e}finally{clearTimeout(i)}}var We=new class{_state;_listeners;constructor(e){this._state={...e},this._listeners=new Map}get(e){return this._state[e]}set(e,t){this._state[e]=t;let n=this._listeners.get(e)??[];for(let e of n)e(t)}subscribe(e,t){return this._listeners.has(e)||this._listeners.set(e,[]),this._listeners.get(e).push(t),()=>{let n=this._listeners.get(e);if(!n)return;let r=n.indexOf(t);r>-1&&n.splice(r,1)}}getState(){return{...this._state}}}({currentUser:null,currentExtensions:[],currentTab:`dashboard`,isAuthenticated:!1,autoRefreshInterval:null});function _(e){return String(e).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`).replace(/'/g,`&#39;`)}async function Ge(e){try{await navigator.clipboard.writeText(e),y(`License data copied to clipboard!`,`success`)}catch(e){console.error(`Error copying to clipboard:`,e),y(`Failed to copy to clipboard`,`error`)}}function Ke(e){if(!e)return``;try{return new Date(e).toLocaleString()}catch{return e}}function qe(e,t){return e.length<=t?e:e.substring(0,t)+`...`}function Je(e){let t=new Date().getTime()-new Date(e).getTime(),n=Math.floor(t/(1e3*60*60)),r=Math.floor(t%(1e3*60*60)/(1e3*60));return n>0?`${n}h ${r}m`:`${r}m`}function Ye(e){return{registered:`<span class="badge" style="background: #10b981;">&#x2705; Registered</span>`,unregistered:`<span class="badge" style="background: #6b7280;">&#x26AA; Unregistered</span>`,failed:`<span class="badge" style="background: #ef4444;">&#x274C; Failed</span>`,disabled:`<span class="badge" style="background: #9ca3af;">&#x23F8;&#xFE0F; Disabled</span>`,degraded:`<span class="badge" style="background: #f59e0b;">&#x26A0;&#xFE0F; Degraded</span>`}[e]||e}function Xe(e){return{healthy:`<span class="badge" style="background: #10b981;">&#x1F49A; Healthy</span>`,warning:`<span class="badge" style="background: #f59e0b;">&#x26A0;&#xFE0F; Warning</span>`,critical:`<span class="badge" style="background: #f59e0b;">&#x1F534; Critical</span>`,down:`<span class="badge" style="background: #ef4444;">&#x1F480; Down</span>`}[e]||e}function Ze(e){return{1:`<span class="badge" style="background: #ef4444;">1 - Highest</span>`,2:`<span class="badge" style="background: #f97316;">2 - High</span>`,3:`<span class="badge" style="background: #eab308;">3 - Medium</span>`,4:`<span class="badge" style="background: #3b82f6;">4 - Low</span>`,5:`<span class="badge" style="background: #6b7280;">5 - Lowest</span>`}[e]||`<span class="badge">${e}</span>`}function Qe(e){return e>=4.3?`quality-excellent`:e>=4?`quality-good`:e>=3.6?`quality-fair`:e>=3.1?`quality-poor`:`quality-bad`}function $e(e){let t=[];if(e.days_of_week){let n=[`Mon`,`Tue`,`Wed`,`Thu`,`Fri`,`Sat`,`Sun`],r=e.days_of_week.map(e=>n[e]).join(`, `);t.push(r)}return e.start_time&&e.end_time&&t.push(`${e.start_time}-${e.end_time}`),e.holidays===!0?t.push(`Holidays`):e.holidays===!1&&t.push(`Non-holidays`),t.length>0?t.join(` | `):`Always`}function et(e){let t=JSON.stringify(e,null,2),n=new Blob([t],{type:`application/json`}),r=URL.createObjectURL(n),i=document.createElement(`a`);i.href=r,i.download=`license_${e.issued_to.replace(/[^a-zA-Z0-9]/g,`_`).toLowerCase()}_${new Date().toISOString().split(`T`)[0]}.json`,document.body.appendChild(i),i.click(),document.body.removeChild(i),URL.revokeObjectURL(r)}window.formatDate=Ke,window.truncate=qe,window.getDuration=Je,window.getStatusBadge=Ye,window.getHealthBadge=Xe,window.getPriorityBadge=Ze,window.getQualityClass=Qe,window.getScheduleDescription=$e,window.downloadLicense=et;var v={enabled:!0,displayTime:8e3,maxErrors:5,showStackTrace:!0},tt=[],nt=!1;function rt(e){nt=e}function y(e,t=`info`){if(nt&&t===`error`){debugLog(`[${t.toUpperCase()}] ${e}`);return}let n=document.createElement(`div`);n.className=`notification notification-${t}`,n.style.cssText=`
        position: fixed;
        top: 80px;
        right: 20px;
        max-width: 400px;
        ...
        font-size: 14px;
        line-height: 1.4;
    `,n.innerHTML=`<strong>${t===`success`?`✓`:t===`error`?`✗`:t===`warning`?`⚠`:`ℹ`}</strong> ${_(e)}`,document.body.appendChild(n),setTimeout(()=>{n.style.animation=`slideOutRight 0.3s ease-in`,setTimeout(()=>n.remove(),300)},5e3)}function b(e,t=``){if(!v.enabled)return;let n=`error-${Date.now()}`,r=e.message||e.toString(),i=e.stack||``;tt.push({id:n,message:r,context:t,stack:i,timestamp:new Date}),tt.length>v.maxErrors&&tt.shift();let a=document.createElement(`div`);a.id=n,a.className=`error-notification`,a.style.cssText=`
        position: fixed;
        top: 70px;
        right: 20px;
        max-width: 450px;
    ...
    `,a.innerHTML=o;let s=a.querySelector(`#close-${n}`);s&&s.addEventListener(`click`,()=>a.remove()),document.body.appendChild(a),setTimeout(()=>{document.getElementById(n)&&(a.style.animation=`slideOut 0.3s ease-in`,setTimeout(()=>a.remove(),300))},v.displayTime),console.error(`[${t||`Error`}]`,r),i&&console.error(`Stack trace:`,i)}var it=1e4,x=null;function at(){typeof window.loadEmergencyContacts==`function`&&window.loadEmergencyContacts(),typeof window.loadEmergencyHistory==`function`&&window.loadEmergencyHistory()}function ot(){typeof window.loadFraudAlerts==`function`&&window.loadFraudAlerts()}function st(){typeof window.loadCallbackQueue==`function`&&window.loadCallbackQueue()}function ct(e){x&&=(clearInterval(x),null);let t={dashboard:()=>window.loadDashboard?.(),analytics:()=>window.loadAnalytics?.(),calls:()=>window.loadCalls?.(),qos:()=>window.loadQoSMetrics?.(),emergency:at,"callback-queue":st,extensions:()=>window.loadExtensions?.(),phones:()=>window.loadRegisteredPhones?.(),atas:()=>window.loadRegisteredATAs?.(),"hot-desking":()=>window.loadHotDeskSessions?.(),voicemail:()=>window.loadVoicemailTab?.(),"fraud-detection":ot,"sbc-management":()=>window.loadSBCData?.()};t[e]&&(x=setInterval(()=>{try{let n=t[e];typeof n==`function`?n():console.error(`Auto-refresh function for ${e} is not a function:`,n)}catch(t){console.error(`Error during auto-refresh of ${e}:`,t),t instanceof Error&&t.message?.includes(`401`)&&debugWarn(`Authentication error during auto-refresh - user may need to re-login`)}},it)),We.set(`autoRefreshInterval`,x)}function S(e){for(let e of document.querySelectorAll(`.tab-content`))e.classList.remove(`active`);for(let e of document.querySelectorAll(`.tab-button`))e.classList.remove(`active`);let t=document.getElementById(e);if(!t)console.error(`CRITICAL: Tab element with id '${e}' not found in DOM`),console.error(`This may indicate a UI template issue or incorrect tab name`),console.error(`Current tab name: "${e}"`);else{t.classList.add(`active`);let n=document.querySelector(`[data-tab="${e}"]`);n?n.classList.add(`active`):debugWarn(`Tab button for '${e}' not found`)}We.set(`currentTab`,e),ct(e);let n={dashboard:window.loadDashboard,analytics:window.loadAnalytics,extensions:window.loadExtensions,phones:window.loadRegisteredPhones,atas:window.loadRegisteredATAs,provisioning:window.loadProvisioning,"auto-attendant":window.loadAutoAttendantConfig,voicemail:window.loadVoicemailTab,paging:window.loadPagingData,calls:window.loadCalls,config:window.loadConfig,"features-status":window.loadFeaturesStatus,"webrtc-phone":window.loadWebRTCPhoneConfig,"license-management":window.initLicenseManagement,qos:window.loadQoSMetrics,"find-me-follow-me":window.loadFMFMExtensions,"time-routing":window.loadTimeRoutingRules,webhooks:window.loadWebhooks,"hot-desking":window.loadHotDeskSessions,"recording-retention":window.loadRetentionPolicies,"jitsi-integration":window.loadJitsiConfig,"matrix-integration":window.loadMatrixConfig,"espocrm-integration":window.loadEspoCRMConfig,"click-to-dial":window.loadClickToDialTab,"fraud-detection":window.loadFraudDetectionData,"nomadic-e911":window.loadNomadicE911Data,"callback-queue":window.loadCallbackQueue,"mobile-push":window.loadMobilePushConfig,"recording-announcements":window.loadRecordingAnnouncements,"speech-analytics":window.loadSpeechAnalyticsConfigs,compliance:window.loadComplianceData,"crm-integrations":window.loadCRMActivityLog,"opensource-integrations":window.loadOpenSourceIntegrations,"sbc-management":window.loadSBCData},r={emergency:[window.loadEmergencyContacts,window.loadEmergencyHistory],codecs:[window.loadCodecStatus,window.loadDTMFConfig],"sip-trunks":[window.loadSIPTrunks,window.loadTrunkHealth,window.loadInboundRoutes],"least-cost-routing":[window.loadLCRRates,window.loadLCRStatistics]},i=n[e];if(i)i();else{let t=r[e];if(t)for(let e of t)e?.()}}function lt(){let e=document.querySelectorAll(`.tab-button`);for(let t of e)t.addEventListener(`click`,()=>{let e=t.getAttribute(`data-tab`);e&&S(e)});let t=document.querySelector(`.sidebar`);t&&t.addEventListener(`keydown`,e=>{let t=e,n=t.target;if(!n.classList.contains(`tab-button`))return;let r=n.closest(`.sidebar-section`);if(!r)return;let i=Array.from(r.querySelectorAll(`.tab-button`)),a=i.indexOf(n),o=-1;t.key===`ArrowDown`?o=a<i.length-1?a+1:0:t.key===`ArrowUp`?o=a>0?a-1:i.length-1:t.key===`Home`?o=0:t.key===`End`&&(o=i.length-1),o>=0&&(t.preventDefault(),i[o]?.focus())}),document.addEventListener(`keydown`,e=>{if(e.key===`Escape`){let e=document.querySelector(`.modal.active`);e&&e.classList.remove(`active`)}})}async function ut(e,t=5,n=1e3){if(!Array.isArray(e))throw TypeError(`promiseFunctions must be an array`);let r=[];for(let i=0;i<e.length;i+=t){let a=e.slice(i,i+t).map(e=>typeof e==`function`?e():e),o=await Promise.allSettled(a);r.push(...o),i+t<e.length&&await new Promise(e=>setTimeout(e,n))}return r}async function dt(){let e=document.getElementById(`refresh-all-button`);if(!e||e.disabled)return;let t=e.textContent,n=e.disabled;try{e.textContent=`⏳ Refreshing All Tabs...`,e.disabled=!0,window.suppressErrorNotifications=!0,debugLog(`Refreshing all data for ALL tabs...`);let t=[];window.loadDashboard&&t.push(()=>window.loadDashboard()),window.loadADStatus&&t.push(()=>window.loadADStatus()),window.loadAnalytics&&t.push(()=>window.loadAnalytics()),window.loadExtensions&&t.push(()=>window.loadExtensions());let n=(await ut(t,5,1e3)).filter(e=>e.status===`rejected`);n.length>0&&debugLog(`${n.length} refresh operation(s) failed (expected for unavailable features):`,n.map(e=>e.reason?.message??e.reason)),y(`✅ All tabs refreshed successfully`,`success`)}catch(e){let t=e instanceof Error?e.message:String(e);console.error(`Error refreshing data:`,e),y(`Failed to refresh: ${t}`,`error`)}finally{window.suppressErrorNotifications=!1,e.textContent=t,e.disabled=n}}function ft(){let e=document.getElementById(`refresh-all-button`);e&&e.addEventListener(`click`,dt)}window.executeBatched=ut,window.refreshAllData=dt;var pt=6e4;async function mt(){try{let e=await g(`${m()}/api/status`,{headers:h()});if(!e.ok)throw Error(`HTTP ${e.status}: ${e.statusText}`);let t=await e.json();document.getElementById(`stat-extensions`).textContent=String(t.registered_extensions??0),document.getElementById(`stat-calls`).textContent=String(t.active_calls??0),document.getElementById(`stat-total-calls`).textContent=String(t.total_calls??0),document.getElementById(`stat-recordings`).textContent=String(t.active_recordings??0);let n=document.getElementById(`system-status`);n&&(n.textContent=`System: ${t.running?`Running`:`Stopped`}`,n.classList.remove(`connected`,`disconnected`),n.classList.add(`status-badge`,t.running?`connected`:`disconnected`)),C()}catch(e){console.error(`Error loading dashboard:`,e);for(let e of[`stat-extensions`,`stat-calls`,`stat-total-calls`,`stat-recordings`]){let t=document.getElementById(e);t&&(t.textContent=`Error`)}y(`Failed to load dashboard: ${e instanceof Error?e.message:String(e)}`,`error`)}}function ht(){mt(),y(`Dashboard refreshed`,`success`)}async function C(){try{let e=await g(`${m()}/api/integrations/ad/status`,{headers:h()});if(!e.ok)throw Error(`HTTP ${e.status}`);let t=await e.json(),n=document.getElementById(`ad-status-badge`);n&&(n.textContent=t.enabled?`Enabled`:`Disabled`,n.className=`status-badge ${t.enabled?`enabled`:`disabled`}`);let r=document.getElementById(`ad-connection-status`);r&&(r.textContent=t.connected?`✓ Connected`:`✗ Not Connected`,r.style.color=t.connected?`#10b981`:`#ef4444`);let i=e=>document.getElementById(e);i(`ad-server`)&&(i(`ad-server`).textContent=t.server??`Not configured`),i(`ad-auto-provision`)&&(i(`ad-auto-provision`).textContent=t.auto_provision?`Yes`:`No`),i(`ad-synced-users`)&&(i(`ad-synced-users`).textContent=String(t.synced_users??0));let a=i(`ad-error`);a&&(a.textContent=t.error??`None`,a.style.color=t.error?`#d32f2f`:`#10b981`);let o=i(`ad-sync-btn`);o&&(o.disabled=!(t.enabled&&t.connected))}catch(e){console.error(`Error loading AD status:`,e)}}function gt(){C(),y(`AD status refreshed`,`success`)}async function _t(){let e=document.getElementById(`ad-sync-btn`);if(!e)return;let t=e.textContent;e.disabled=!0,e.textContent=`Syncing...`;try{let e=await g(`${m()}/api/integrations/ad/sync`,{method:`POST`,headers:h()},pt);if(!e.ok){let t=await e.json().catch(()=>({error:`HTTP ${e.status}`}));throw Error(t.error||`HTTP ${e.status}`)}let t=await e.json();t.success?(y(t.message||`Successfully synced ${t.synced_count} users`,`success`),C()):y(t.error||`Failed to sync users`,`error`)}catch(e){console.error(`Error syncing AD users:`,e),y((e instanceof Error?e.message:String(e))===`Request timed out`?`AD sync timed out. Check server logs.`:`Error syncing AD users`,`error`)}finally{e.textContent=t,e.disabled=!1}}window.loadDashboard=mt,window.refreshDashboard=ht,window.loadADStatus=C,window.refreshADStatus=gt,window.syncADUsers=_t;var vt=1e4;async function w(){let e=document.getElementById(`extensions-table-body`);if(e){e.innerHTML=`<tr><td colspan="8" class="loading">Loading extensions...</td></tr>`;try{let t=await g(`${m()}/api/extensions`,{headers:h()},vt);if(!t.ok)throw Error(`HTTP error! status: ${t.status}`);let n=await t.json();if(window.currentExtensions=n,n.length===0){e.innerHTML=`<tr><td colspan="8" class="loading">No extensions found.</td></tr>`;return}let r=e=>{let t=``;return e.ad_synced&&(t+=` <span class="ad-badge" title="Synced from Active Directory">AD</span>`),e.is_admin&&(t+=` <span class="admin-badge" title="Admin Privileges">Admin</span>`),t};e.innerHTML=n.map(e=>`
        ...
        `).join(``)}catch(t){console.error(`Error loading extensions:`,t),e.innerHTML=`<tr><td colspan="8" class="loading">${(t instanceof Error?t.message:String(t))===`Request timed out`?`Request timed out. System may still be starting.`:`Error loading extensions`}</td></tr>`}}}function yt(){let e=document.getElementById(`add-extension-modal`);e&&e.classList.add(`active`);let t=document.getElementById(`add-extension-form`);t&&t.reset()}function bt(){let e=document.getElementById(`add-extension-modal`);e&&e.classList.remove(`active`)}function xt(e){let t=(window.currentExtensions??[]).find(t=>t.number===e);if(!t)return;let n=e=>document.getElementById(e);n(`edit-ext-number`)&&(n(`edit-ext-number`).value=t.number),n(`edit-ext-name`)&&(n(`edit-ext-name`).value=t.name),n(`edit-ext-email`)&&(n(`edit-ext-email`).value=t.email??``),n(`edit-ext-did-number`)&&(n(`edit-ext-did-number`).value=t.did_number??``),n(`edit-ext-allow-external`)&&(n(`edit-ext-allow-external`).checked=!!t.allow_external),n(`edit-ext-is-admin`)&&(n(`edit-ext-is-admin`).checked=!!t.is_admin),n(`edit-ext-password`)&&(n(`edit-ext-password`).value=``);let r=document.getElementById(`edit-extension-modal`);r&&r.classList.add(`active`)}function St(){let e=document.getElementById(`edit-extension-modal`);e&&e.classList.remove(`active`)}async function Ct(e){if(confirm(`Are you sure you want to delete extension ${e}?`))try{let t=m(),n=await fetch(`${t}/api/extensions/${e}`,{method:`DELETE`,headers:h()});n.ok?(y(`Extension deleted successfully`,`success`),w()):y((await n.json()).error||`Failed to delete extension`,`error`)}catch(e){console.error(`Error deleting extension:`,e),y(`Failed to delete extension`,`error`)}}async function wt(e){if(confirm(`Reboot phone for extension ${e}?`))try{let t=m();(await fetch(`${t}/api/phones/${e}/reboot`,{method:`POST`,headers:h()})).ok?y(`Reboot command sent to ${e}`,`success`):y(`Failed to reboot phone`,`error`)}catch(e){console.error(`Error rebooting phone:`,e),y(`Failed to reboot phone`,`error`)}}async function Tt(){if(confirm(`Reboot ALL registered phones?`))try{let e=m();(await fetch(`${e}/api/phones/reboot`,{method:`POST`,headers:h()})).ok?y(`Reboot command sent to all phones`,`success`):y(`Failed to reboot phones`,`error`)}catch(e){console.error(`Error rebooting all phones:`,e),y(`Failed to reboot phones`,`error`)}}function Et(){let e=document.getElementById(`add-extension-form`);e&&e.addEventListener(`submit`,async e=>{e.preventDefault();let t=e=>document.getElementById(e)?.value??``,n=e=>document.getElementById(e)?.checked??!1,r={number:t(`new-ext-number`),name:t(`new-ext-name`),email:t(`new-ext-email`),password:t(`new-ext-password`),voicemail_pin:t(`new-ext-voicemail-pin`),did_number:t(`new-ext-did-number`),allow_external:n(`new-ext-allow-external`),is_admin:n(`new-ext-is-admin`)};try{let e=m(),t=await fetch(`${e}/api/extensions`,{method:`POST`,headers:{...h(),"Content-Type":`application/json`},body:JSON.stringify(r)}),n=await t.json();t.ok&&n.success?(y(`Extension added successfully`,`success`),bt(),w()):y(n.error||`Failed to add extension`,`error`)}catch(e){console.error(`Error adding extension:`,e),y(`Failed to add extension`,`error`)}});let t=document.getElementById(`edit-extension-form`);t&&t.addEventListener(`submit`,async e=>{e.preventDefault();let t=e=>document.getElementById(e)?.value??``,n=e=>document.getElementById(e)?.checked??!1,r=t(`edit-ext-number`),i=t(`edit-ext-password`),a=t(`edit-ext-voicemail-pin`),o={name:t(`edit-ext-name`),email:t(`edit-ext-email`),did_number:t(`edit-ext-did-number`),allow_external:n(`edit-ext-allow-external`),is_admin:n(`edit-ext-is-admin`)};i&&(o.password=i),a&&(o.voicemail_pin=a);try{let e=m(),t=await fetch(`${e}/api/extensions/${r}`,{method:`PUT`,headers:{...h(),"Content-Type":`application/json`},body:JSON.stringify(o)}),n=await t.json();t.ok&&n.success?(y(`Extension updated successfully`,`success`),St(),w()):y(n.error||`Failed to update extension`,`error`)}catch(e){console.error(`Error updating extension:`,e),y(`Failed to update extension`,`error`)}})}window.loadExtensions=w,window.showAddExtensionModal=yt,window.closeAddExtensionModal=bt,window.editExtension=xt,window.closeEditExtensionModal=St,window.deleteExtension=Ct,window.rebootPhone=wt,window.rebootAllPhones=Tt,document.readyState===`loading`?document.addEventListener(`DOMContentLoaded`,Et):Et();async function Dt(){try{let e=m(),t=await fetch(`${e}/api/extensions`,{headers:h()});if(!t.ok)throw Error(`HTTP error! status: ${t.status}`);let n=await t.json(),r=document.getElementById(`vm-extension-select`);if(!r)return;r.innerHTML=`<option value="">Select Extension</option>`;for(let e of n){let t=document.createElement(`option`);t.value=e.number,t.textContent=`${e.number} - ${e.name}`,r.appendChild(t)}}catch(e){console.error(`Error loading voicemail tab:`,e),y(`Failed to load extensions`,`error`)}}async function Ot(){let e=document.getElementById(`vm-extension-select`)?.value;if(!e){for(let e of[`voicemail-pin-section`,`voicemail-messages-section`,`voicemail-box-overview`]){let t=document.getElementById(e);t&&(t.style.display=`none`)}return}for(let e of[`voicemail-pin-section`,`voicemail-messages-section`,`voicemail-box-overview`]){let t=document.getElementById(e);t&&(t.style.display=`block`)}let t=document.getElementById(`vm-current-extension`);t&&(t.textContent=e);try{let t=m(),n=await fetch(`${t}/api/voicemail/${e}`,{headers:h()});if(!n.ok)throw Error(`HTTP error! status: ${n.status}`);kt((await n.json()).messages,e)}catch(e){console.error(`Error loading voicemail:`,e),y(`Failed to load voicemail messages`,`error`)}}function kt(e,t){let n=document.getElementById(`voicemail-cards-view`);if(n){if(!e||e.length===0){n.innerHTML=`<div class="info-box">No voicemail messages</div>`;return}n.innerHTML=e.map(e=>{let n=new Date(e.timestamp).toLocaleString(),r=e.duration?`${e.duration}s`:`Unknown`,i=!e.listened;return`
        ...
        `}).join(``)}}async function At(e,t){try{let n=`${m()}/api/voicemail/${e}/${t}/audio`,r=document.getElementById(`vm-audio-player`);r&&(r.src=n,r.play()),await Mt(e,t)}catch(e){console.error(`Error playing voicemail:`,e),y(`Failed to play voicemail`,`error`)}}async function jt(e,t){let n=m();window.open(`${n}/api/voicemail/${e}/${t}/audio?download=1`,`_blank`)}async function Mt(e,t){try{let n=m();await fetch(`${n}/api/voicemail/${e}/${t}/read`,{method:`POST`,headers:h()})}catch(e){console.error(`Error marking voicemail read:`,e)}}async function Nt(e,t){if(confirm(`Delete this voicemail message?`))try{let n=m();(await fetch(`${n}/api/voicemail/${e}/${t}`,{method:`DELETE`,headers:h()})).ok?(y(`Voicemail deleted`,`success`),Ot()):y(`Failed to delete voicemail`,`error`)}catch(e){console.error(`Error deleting voicemail:`,e),y(`Failed to delete voicemail`,`error`)}}function Pt(){let e=document.getElementById(`vm-audio-player`);e&&(e.pause(),e.src=``);let t=document.getElementById(`voicemail-player-section`);t&&(t.style.display=`none`)}function Ft(){let e=document.getElementById(`voicemail-cards-view`),t=document.getElementById(`voicemail-table-view`),n=document.getElementById(`toggle-voicemail-view-btn`);if(e&&t){let r=e.style.display!==`none`;e.style.display=r?`none`:`block`,t.style.display=r?`block`:`none`,n&&(n.textContent=r?`Switch to Card View`:`Switch to Table View`)}}window.loadVoicemailTab=Dt,window.loadVoicemailForExtension=Ot,window.playVoicemail=At,window.downloadVoicemail=jt,window.deleteVoicemail=Nt,window.markVoicemailRead=Mt,window.closeVoicemailPlayer=Pt,window.toggleVoicemailView=Ft;async function It(){let e=document.getElementById(`calls-list`);if(e){e.innerHTML=`<div class="loading">Loading calls...</div>`;try{let t=await(await g(`${m()}/api/calls`,{headers:h()})).json();if(t.length===0){e.innerHTML=`<div class="loading">No active calls</div>`;return}e.innerHTML=t.map(e=>`
            <div class="call-item"><strong>Call:</strong> ${_(String(e))}</div>
        `).join(``)}catch(t){console.error(`Error loading calls:`,t),e.innerHTML=`<div class="loading">Error loading calls</div>`}}}async function Lt(){try{let e=await g(`${m()}/api/config/codecs`,{headers:h()});if(!e.ok)throw Error(`Failed to load codecs (HTTP ${e.status})`);let t=await e.json(),n=document.getElementById(`codec-status`);n&&(t.codecs&&t.codecs.length>0?n.innerHTML=t.codecs.map(e=>`<div class="codec-item">${_(e.name)} - <span class="status-${e.enabled?`enabled`:`disabled`}">${e.enabled?`Enabled`:`Disabled`}</span></div>`).join(``):n.innerHTML=`<div class="info-box">No codecs configured</div>`)}catch(e){console.error(`Error loading codec status:`,e);let t=document.getElementById(`codec-status`);t&&(t.innerHTML=`<div class="error-box">Failed to load codec configuration</div>`)}}async function Rt(){try{let e=await g(`${m()}/api/config/dtmf`,{headers:h()});if(!e.ok)throw Error(`HTTP ${e.status}`);let t=await e.json(),n=document.getElementById(`dtmf-mode`);n&&(n.value=t.mode??`rfc2833`);let r=document.getElementById(`dtmf-detection-threshold`);r&&(r.value=String(t.threshold??-30))}catch(e){console.error(`Error loading DTMF config:`,e)}}async function zt(){try{let e=m(),t={mode:document.getElementById(`dtmf-mode`)?.value??`rfc2833`,threshold:parseInt(document.getElementById(`dtmf-detection-threshold`)?.value??`-30`)};(await fetch(`${e}/api/config/dtmf`,{method:`POST`,headers:{...h(),"Content-Type":`application/json`},body:JSON.stringify(t)})).ok?y(`DTMF configuration saved`,`success`):y(`Failed to save DTMF configuration`,`error`)}catch(e){console.error(`Error saving DTMF config:`,e),y(`Failed to save DTMF configuration`,`error`)}}async function Bt(e){e&&e.preventDefault();try{let e=m(),t=document.getElementById(`codec-config-form`);if(!t)return;let n=new FormData(t),r=Object.fromEntries(n.entries());(await fetch(`${e}/api/config/codecs`,{method:`POST`,headers:{...h(),"Content-Type":`application/json`},body:JSON.stringify(r)})).ok?y(`Codec configuration saved`,`success`):y(`Failed to save codec configuration`,`error`)}catch(e){console.error(`Error saving codec config:`,e),y(`Failed to save codec configuration`,`error`)}}window.loadCalls=It,window.loadCodecStatus=Lt,window.loadDTMFConfig=Rt,window.saveDTMFConfig=zt,window.saveCodecConfig=Bt;var Vt=`Configuration saved successfully. Restart may be required for some changes.`;async function Ht(){try{let e=await g(`${m()}/api/config/full`,{headers:h()});if(!e.ok)throw Error(`HTTP ${e.status}`);let t=await e.json();if(t.features)for(let e of[`call-recording`,`call-transfer`,`call-hold`,`conference`,`voicemail`,`call-parking`,`call-queues`,`presence`,`music-on-hold`,`auto-attendant`]){let n=document.getElementById(`feature-${e}`),r=e.replace(/-/g,`_`);n&&(n.checked=t.features[r]??!1)}if(t.voicemail){let e=e=>document.getElementById(e);e(`voicemail-max-duration`)&&(e(`voicemail-max-duration`).value=String(t.voicemail.max_duration??120))}}catch(e){console.error(`Error loading config:`,e),y(`Failed to load configuration`,`error`)}}async function Ut(){try{let e=await g(`${m()}/api/config/features`,{headers:h()});if(!e.ok)throw Error(`HTTP ${e.status}`);let t=await e.json();Wt(`core-features-table`,t.core),Wt(`advanced-features-table`,t.advanced),Wt(`integration-features-table`,t.integrations)}catch(e){console.error(`Error loading features status:`,e),y(`Failed to load feature status`,`error`)}}function Wt(e,t){let n=document.getElementById(e);if(n){if(!t||Object.keys(t).length===0){n.innerHTML=`<tr><td colspan="3" style="text-align:center;">No features found</td></tr>`;return}n.innerHTML=Object.entries(t).map(([e,t])=>{let n=e.replace(/_/g,` `).replace(/\b\w/g,e=>e.toUpperCase()),r=t.enabled?`<span class="badge" style="background:#10b981;">Enabled</span>`:`<span class="badge" style="background:#6b7280;">Disabled</span>`;return`<tr>
            <td><strong>${_(n)}</strong></td>
            <td>${r}</td>
            <td>${_(t.description)}</td>
        </tr>`}).join(``)}}async function Gt(e){try{let t=m(),n=document.getElementById(`${e}-form`);if(!n)return;let r=new FormData(n),i=Object.fromEntries(r.entries()),a=await fetch(`${t}/api/config/section`,{method:`PUT`,headers:{...h(),"Content-Type":`application/json`},body:JSON.stringify({section:e,data:i})});a.ok?y(Vt,`success`):y((await a.json()).error||`Failed to save configuration`,`error`)}catch(t){console.error(`Error saving ${e} config:`,t),y(`Failed to save ${e} configuration`,`error`)}}async function Kt(){try{let e=await g(`${m()}/api/ssl/status`,{headers:h()});if(!e.ok)throw Error(`HTTP ${e.status}`);let t=await e.json(),n=document.getElementById(`ssl-status-info`);if(n&&(n.textContent=t.enabled?`Enabled`:`Disabled`,n.className=`status-badge ${t.enabled?`enabled`:`disabled`}`),t.certificate){let e=document.getElementById(`cert-details-section`);e&&(e.style.display=`block`);let n=document.getElementById(`cert-subject`),r=document.getElementById(`cert-issuer`),i=document.getElementById(`cert-valid-from`),a=document.getElementById(`cert-valid-until`);n&&(n.value=t.certificate.subject||`N/A`),r&&(r.value=t.certificate.issuer||`N/A`),i&&(i.value=t.certificate.expires||`N/A`),a&&(a.value=t.certificate.expires||`N/A`)}}catch(e){console.error(`Error loading SSL status:`,e)}}async function qt(){try{let e=m(),t=await fetch(`${e}/api/ssl/generate-certificate`,{method:`POST`,headers:{...h(),"Content-Type":`application/json`},body:JSON.stringify({})});t.ok?(y(`SSL certificate generated successfully. Server restart required.`,`success`),Kt()):y((await t.json()).error||`Failed to generate SSL certificate`,`error`)}catch(e){console.error(`Error generating SSL certificate:`,e),y(`Failed to generate SSL certificate`,`error`)}}async function Jt(){await Kt(),y(`SSL status refreshed`,`success`)}var Yt=[`features-config`,`voicemail-config`,`email-config`,`recording-config`,`security-config`,`advanced-features`,`conference-config`,`ssl-config`];function Xt(){for(let e of Yt){let t=document.getElementById(`${e}-form`);t&&t.addEventListener(`submit`,async t=>{t.preventDefault(),await Gt(e)})}}window.loadConfig=Ht,window.loadFeaturesStatus=Ut,window.saveConfigSection=Gt,window.loadSSLStatus=Kt,window.generateSSLCertificate=qt,window.refreshSSLStatus=Jt,document.readyState===`loading`?document.addEventListener(`DOMContentLoaded`,Xt):Xt();var Zt=[],T={};async function Qt(){await Promise.all([$t(),en(),tn(),nn(),rn(),an()])}async function $t(){try{let e=m(),t=await fetch(`${e}/api/provisioning/vendors`,{headers:h()});if(!t.ok)throw Error(`HTTP ${t.status}`);let n=await t.json();Zt=n.vendors||[],T=n.models||{},sn(),cn()}catch(e){console.error(`Error loading vendors:`,e);let t=document.getElementById(`supported-vendors-list`);t&&(t.innerHTML=`<p>Failed to load supported vendors.</p>`)}}async function en(){try{let e=m(),t=await fetch(`${e}/api/extensions`,{headers:h()});if(!t.ok)throw Error(`HTTP ${t.status}`);let n=await t.json(),r=document.getElementById(`device-extension`);if(!r)return;r.innerHTML=`<option value="">Select Extension</option>`;for(let e of n){let t=document.createElement(`option`);t.value=e.number,t.textContent=`${e.number} - ${e.name}`,r.appendChild(t)}}catch(e){console.error(`Error loading extensions for provisioning:`,e)}}async function tn(){try{let e=m(),t=await fetch(`${e}/api/provisioning/devices`,{headers:h()});if(!t.ok)throw Error(`HTTP ${t.status}`);let n=await t.json(),r=document.getElementById(`provisioning-devices-table-body`);if(!r)return;let i=Array.isArray(n)?n:n.devices||[];if(i.length===0){r.innerHTML=`<tr><td colspan="8">No provisioned devices</td></tr>`;return}r.innerHTML=i.map(e=>`
            <tr>
        ...
        `).join(``)}catch(e){console.error(`Error loading devices:`,e)}}async function nn(){try{let e=m(),t=await fetch(`${e}/api/provisioning/templates`,{headers:h()});if(!t.ok)throw Error(`HTTP ${t.status}`);let n=await t.json(),r=document.getElementById(`templates-table-body`);if(!r)return;let i=n.templates||[];if(i.length===0){r.innerHTML=`<tr><td colspan="5">No templates configured</td></tr>`;return}r.innerHTML=i.map(e=>{let t=e.name||`${e.vendor||`unknown`}_${e.model||`unknown`}`;return`
            ...
            </tr>`}).join(``)}catch(e){console.error(`Error loading templates:`,e)}}async function rn(){try{let e=m(),t=await fetch(`${e}/api/provisioning/settings`,{headers:h()});if(!t.ok)return;let n=await t.json(),r=e=>document.getElementById(e);r(`provisioning-enabled`)&&(r(`provisioning-enabled`).checked=n.enabled??!1),r(`provisioning-url-format`)&&(r(`provisioning-url-format`).value=n.url_format??``),un()}catch(e){console.error(`Error loading provisioning settings:`,e)}}async function an(){try{let e=m(),t=await fetch(`${e}/api/provisioning/phonebook-settings`,{headers:h()});if(!t.ok)return;let n=await t.json(),r=e=>document.getElementById(e);r(`ldap-phonebook-enabled`)&&(r(`ldap-phonebook-enabled`).checked=n.ldap_enabled??!1),r(`remote-phonebook-enabled`)&&(r(`remote-phonebook-enabled`).checked=n.remote_enabled??!1),fn(),pn()}catch(e){console.error(`Error loading phonebook settings:`,e)}}async function on(e){if(confirm(`Delete device ${e}?`))try{let t=m();(await fetch(`${t}/api/provisioning/devices/${e}`,{method:`DELETE`,headers:h()})).ok?(y(`Device deleted`,`success`),tn()):y(`Failed to delete device`,`error`)}catch(e){console.error(`Error deleting device:`,e),y(`Failed to delete device`,`error`)}}function sn(){let e=document.getElementById(`device-vendor`);if(e){e.innerHTML=`<option value="">Select Vendor</option>`;for(let t of Zt){let n=document.createElement(`option`);n.value=t,n.textContent=t,e.appendChild(n)}}}function cn(){let e=document.getElementById(`supported-vendors-list`);if(e){if(Zt.length===0){e.innerHTML=`<p>No supported vendors found.</p>`;return}e.innerHTML=Zt.map(e=>{let t=T[e]||T[e.toLowerCase()]||[],n=t.length>0?t.map(e=>`<span class="badge">${_(e.toUpperCase())}</span>`).join(` `):`<em>No models</em>`;return`<div style="margin-bottom: 8px;">
            <strong>${_(e.charAt(0).toUpperCase()+e.slice(1))}</strong>: ${n}
        </div>`}).join(``)}}function ln(){let e=document.getElementById(`device-vendor`),t=document.getElementById(`device-model`);if(!e||!t)return;let n=e.value;if(t.innerHTML=``,!n){t.innerHTML=`<option value="">Select Vendor First</option>`;return}let r=T[n]||T[n.toLowerCase()]||[];if(r.length===0){t.innerHTML=`<option value="">No models available</option>`;return}t.innerHTML=`<option value="">Select Model</option>`;for(let e of r){let n=document.createElement(`option`);n.value=e,n.textContent=e,t.appendChild(n)}}function un(){let e=document.getElementById(`provisioning-enabled`),t=document.getElementById(`provisioning-settings`);e&&t&&(t.style.display=e.checked?`block`:`none`)}async function dn(){try{let e=e=>document.getElementById(e)?.value??``,t={enabled:(e=>document.getElementById(e)?.checked??!1)(`provisioning-enabled`),server_ip:e(`provisioning-server-ip`),port:parseInt(e(`provisioning-port`),10)||9e3,custom_templates_dir:e(`provisioning-custom-dir`)},n=m();(await fetch(`${n}/api/provisioning/settings`,{method:`PUT`,headers:{...h(),"Content-Type":`application/json`},body:JSON.stringify(t)})).ok?y(`Provisioning settings saved`,`success`):y(`Failed to save provisioning settings`,`error`)}catch(e){console.error(`Error saving provisioning settings:`,e),y(`Failed to save provisioning settings`,`error`)}}function fn(){let e=document.getElementById(`ldap-phonebook-enabled`),t=document.getElementById(`ldap-phonebook-settings`);e&&t&&(t.style.display=e.checked?`block`:`none`)}function pn(){let e=document.getElementById(`remote-phonebook-enabled`),t=document.getElementById(`remote-phonebook-settings`);e&&t&&(t.style.display=e.checked?`block`:`none`)}async function mn(){try{let e=e=>document.getElementById(e)?.value??``,t=e=>document.getElementById(e)?.checked??!1,n={ldap_enabled:t(`ldap-phonebook-enabled`),ldap_server:e(`ldap-phonebook-server`),ldap_port:parseInt(e(`ldap-phonebook-port`),10)||636,ldap_base_dn:e(`ldap-phonebook-base`),ldap_bind_user:e(`ldap-phonebook-user`),ldap_bind_password:e(`ldap-phonebook-password`),ldap_use_tls:t(`ldap-phonebook-tls`),ldap_display_name:e(`ldap-phonebook-display-name`),remote_enabled:t(`remote-phonebook-enabled`),remote_refresh_interval:parseInt(e(`remote-phonebook-refresh`),10)||60},r=m();(await fetch(`${r}/api/provisioning/phonebook-settings`,{method:`PUT`,headers:{...h(),"Content-Type":`application/json`},body:JSON.stringify(n)})).ok?y(`Phone book settings saved`,`success`):y(`Failed to save phone book settings`,`error`)}catch(e){console.error(`Error saving phonebook settings:`,e),y(`Failed to save phone book settings`,`error`)}}async function hn(){try{let e=m();(await fetch(`${e}/api/provisioning/reload-templates`,{method:`POST`,headers:h()})).ok?(y(`Templates reloaded from disk`,`success`),nn()):y(`Failed to reload templates`,`error`)}catch(e){console.error(`Error reloading templates:`,e),y(`Failed to reload templates`,`error`)}}function gn(){let e=document.getElementById(`add-device-form`);e&&e.reset();let t=document.getElementById(`device-model`);t&&(t.innerHTML=`<option value="">Select Vendor First</option>`)}function _n(e){y(`Viewing template: ${e}`,`info`)}function vn(){let e=document.getElementById(`add-device-form`);e&&e.addEventListener(`submit`,async e=>{e.preventDefault();let t=e=>document.getElementById(e)?.value??``,n={mac_address:t(`device-mac`),extension_number:t(`device-extension`),vendor:t(`device-vendor`),model:t(`device-model`)};if(!n.mac_address||!n.extension_number||!n.vendor||!n.model){y(`Please fill in all required fields`,`error`);return}try{let e=m(),t=await fetch(`${e}/api/provisioning/devices`,{method:`POST`,headers:{...h(),"Content-Type":`application/json`},body:JSON.stringify(n)}),r=await t.json();t.ok&&r.success?(y(r.message||`Device added successfully`,`success`),gn(),tn()):y(r.error||`Failed to add device`,`error`)}catch(e){console.error(`Error adding device:`,e),y(`Failed to add device`,`error`)}})}window.loadProvisioning=Qt,window.loadSupportedVendors=$t,window.loadProvisioningDevices=tn,window.loadProvisioningTemplates=nn,window.loadProvisioningSettings=rn,window.loadPhonebookSettings=an,window.deleteDevice=on,window.viewTemplate=_n,window.updateModelOptions=ln,window.toggleProvisioningEnabled=un,window.saveProvisioningSettings=dn,window.toggleLdapPhonebookSettings=fn,window.toggleRemotePhonebookSettings=pn,window.savePhonebookSettings=mn,window.reloadTemplates=hn,window.resetAddDeviceForm=gn,document.readyState===`loading`?document.addEventListener(`DOMContentLoaded`,vn):vn();async function yn(){let e=document.getElementById(`registered-phones-table-body`);if(e)try{let t=await g(`${m()}/api/registered-phones`,{headers:h()});if(!t.ok)throw Error(`HTTP ${t.status}`);let n=await t.json();if(n.length===0){e.innerHTML=`<tr><td colspan="5">No registered phones</td></tr>`;return}e.innerHTML=n.map(e=>`
            <tr>
        ...
        `).join(``)}catch(t){console.error(`Error loading registered phones:`,t),e.innerHTML=`<tr><td colspan="5">Error loading phones</td></tr>`}}async function bn(){let e=document.getElementById(`registered-atas-table-body`);if(e)try{let t=await g(`${m()}/api/registered-phones/atas`,{headers:h()});if(!t.ok)throw Error(`HTTP ${t.status}`);let n=await t.json();if(n.length===0){e.innerHTML=`<tr><td colspan="6">No registered ATAs</td></tr>`;return}e.innerHTML=n.map(e=>{let t=[e.vendor,e.model].filter(Boolean).join(` `);return`
            <tr>
                <td>${_(e.extension_number||``)}</td>
                <td>${_(e.ip_address||``)}</td>
                <td>${_(e.mac_address||``)}</td>
        ...
        `}).join(``)}catch(t){console.error(`Error loading ATAs:`,t),e.innerHTML=`<tr><td colspan="6">Error loading ATAs</td></tr>`}}window.loadRegisteredPhones=yn,window.loadRegisteredATAs=bn;async function xn(){try{let e=m(),t=await fetch(`${e}/api/fraud-detection/alerts`,{headers:h()});if(!t.ok)throw Error(`HTTP ${t.status}`);let n=await t.json(),r=document.getElementById(`fraud-alerts-list`);if(!r)return;let i=n.alerts??[];if(i.length===0){r.innerHTML=`<div class="info-box">No fraud alerts</div>`;return}r.innerHTML=i.map(e=>`
        ...
        `).join(``)}catch(e){console.error(`Error loading fraud alerts:`,e)}}async function Sn(){try{let e=m(),t=await fetch(`${e}/api/callback-queue/list`,{headers:h()});if(!t.ok)throw Error(`HTTP ${t.status}`);let n=await t.json(),r=document.getElementById(`callback-list`);if(!r)return;let i=n.queue??[];if(i.length===0){r.innerHTML=`<tr><td colspan="5">No callbacks in queue</td></tr>`;return}r.innerHTML=i.map(e=>`
            <tr>
                <td>${_(e.caller||``)}</td>
                <td>${_(e.number||``)}</td>
                <td>${_(e.status||``)}</td>
        ...
        `).join(``)}catch(e){console.error(`Error loading callback queue:`,e)}}async function Cn(e){try{let t=m();(await fetch(`${t}/api/callback-queue/start`,{method:`POST`,headers:{...h(),"Content-Type":`application/json`},body:JSON.stringify({callback_id:e,agent_id:`admin`})})).ok?(y(`Callback initiated`,`success`),Sn()):y(`Failed to start callback`,`error`)}catch(e){console.error(`Error starting callback:`,e),y(`Failed to start callback`,`error`)}}async function wn(e){try{let t=m();(await fetch(`${t}/api/callback-queue/cancel`,{method:`POST`,headers:{...h(),"Content-Type":`application/json`},body:JSON.stringify({callback_id:e})})).ok?(y(`Callback cancelled`,`success`),Sn()):y(`Failed to cancel callback`,`error`)}catch(e){console.error(`Error cancelling callback:`,e),y(`Failed to cancel callback`,`error`)}}async function Tn(){try{let e=m(),t=await fetch(`${e}/api/mobile-push/devices`,{headers:h()});if(!t.ok)throw Error(`HTTP ${t.status}`);let n=await t.json(),r=document.getElementById(`mobile-devices-list`);if(!r)return;let i=n.devices??[];if(i.length===0){r.innerHTML=`<div class="info-box">No registered devices</div>`;return}r.innerHTML=i.map(e=>`
        ...
        `).join(``)}catch(e){console.error(`Error loading mobile push devices:`,e)}}async function En(){try{let e=m(),t=await fetch(`${e}/api/framework/speech-analytics/configs`,{headers:h()});if(!t.ok)return;let n=await t.json(),r=document.getElementById(`speech-analytics-configs-table`);if(r){let e=n.configs??[];e.length===0?r.innerHTML=`<div class="info-box">No speech analytics configurations</div>`:r.innerHTML=`
                    <table class="table">
                        <thead>
                            <tr>
                                <th>Configuration</th>
                ...
                `}}catch(e){console.error(`Error loading speech analytics:`,e)}}window.loadFraudAlerts=xn,window.loadCallbackQueue=Sn,window.startCallback=Cn,window.cancelCallback=wn,window.loadMobilePushDevices=Tn,window.loadSpeechAnalyticsConfigs=En;async function Dn(){try{let e=m(),t=await fetch(`${e}/api/emergency/contacts`,{headers:h()});if(!t.ok)throw Error(`HTTP ${t.status}`);let n=await t.json(),r=document.getElementById(`emergency-contacts-table`);if(!r)return;let i=n.contacts??[];if(i.length===0){r.innerHTML=`<tr><td colspan="5">No emergency contacts</td></tr>`;return}r.innerHTML=i.map(e=>`
            <tr>
                <td>${_(e.name||``)}</td>
                <td>${_(e.phone||``)}</td>
                <td>${_(e.role||``)}</td>
        ...
        `).join(``)}catch(e){console.error(`Error loading emergency contacts:`,e)}}function On(e){return`<span class="status-badge ${{high:`danger`,medium:`warning`,low:`info`}[e??``]??`info`}">${e??`normal`}</span>`}async function kn(){try{let e=m(),t=await fetch(`${e}/api/emergency/history`,{headers:h()});if(!t.ok)return;let n=await t.json(),r=document.getElementById(`emergency-history-table`);if(!r)return;let i=n.history??[];r.innerHTML=i.length===0?`<div class="info-box">No emergency history</div>`:i.map(e=>`
                <div class="history-item">
                    <strong>${new Date(e.timestamp).toLocaleString()}</strong> - ${_(e.description||``)}
                </div>
            `).join(``)}catch(e){console.error(`Error loading emergency history:`,e)}}async function An(e){if(confirm(`Delete this emergency contact?`))try{let t=m();(await fetch(`${t}/api/emergency/contacts/${e}`,{method:`DELETE`,headers:h()})).ok&&(y(`Emergency contact deleted`,`success`),Dn())}catch(e){console.error(`Error deleting contact:`,e),y(`Failed to delete contact`,`error`)}}async function jn(){try{let e=m(),t=await fetch(`${e}/api/framework/nomadic-e911/sites`,{headers:h()});if(!t.ok)throw Error(`HTTP ${t.status}`);let n=await t.json(),r=document.getElementById(`e911-sites-table`);if(!r)return;let i=n.sites??[];r.innerHTML=i.length===0?`<div class="info-box">No E911 sites configured</div>`:i.map(e=>`
                <div class="site-item">
            ...
            `).join(``)}catch(e){console.error(`Error loading E911 sites:`,e);let t=document.getElementById(`e911-sites-table`);t&&(t.innerHTML=`<div class="error-box">Failed to load E911 sites</div>`)}}async function Mn(){try{let e=m(),t=await fetch(`${e}/api/framework/nomadic-e911/locations`,{headers:h()});if(!t.ok)throw Error(`HTTP ${t.status}`);let n=await t.json(),r=document.getElementById(`extension-locations-table`);if(r){let e=n.locations??[];r.innerHTML=e.length===0?`<div class="info-box">No locations assigned</div>`:e.map(e=>`
                    <div class="location-item">
                        Extension ${_(e.extension)} - ${_(e.site_name||`Unassigned`)}
                    </div>
                `).join(``)}}catch(e){console.error(`Error loading extension locations:`,e)}}function Nn(){let e=document.getElementById(`add-emergency-contact-modal`);e&&e.classList.add(`active`)}function Pn(){let e=document.getElementById(`add-emergency-contact-modal`);e&&e.classList.remove(`active`)}function Fn(){let e=document.getElementById(`trigger-emergency-modal`);e&&e.classList.add(`active`)}function In(){let e=document.getElementById(`trigger-emergency-modal`);e&&e.classList.remove(`active`)}async function Ln(e){e.preventDefault();let t=e=>document.getElementById(e)?.value??``,n=e=>document.getElementById(e)?.checked??!1,r=[];n(`method-call`)&&r.push(`call`),n(`method-page`)&&r.push(`page`),n(`method-email`)&&r.push(`email`),n(`method-sms`)&&r.push(`sms`);let i={name:t(`emergency-contact-name`),extension:t(`emergency-contact-extension`),phone:t(`emergency-contact-phone`),email:t(`emergency-contact-email`),priority:t(`emergency-contact-priority`),notification_methods:r};try{let e=m();(await fetch(`${e}/api/emergency/contacts`,{method:`POST`,headers:{...h(),"Content-Type":`application/json`},body:JSON.stringify(i)})).ok?(y(`Emergency contact added`,`success`),Pn(),Dn()):y(`Failed to add emergency contact`,`error`)}catch(e){console.error(`Error adding emergency contact:`,e),y(`Failed to add emergency contact`,`error`)}}async function Rn(e){e.preventDefault();let t=e=>document.getElementById(e)?.value??``,n={type:t(`trigger-type`),details:t(`trigger-details`),info:t(`trigger-info`)};try{let e=m();(await fetch(`${e}/api/emergency/trigger`,{method:`POST`,headers:{...h(),"Content-Type":`application/json`},body:JSON.stringify(n)})).ok?(y(`Emergency notification sent`,`success`),In(),kn()):y(`Failed to trigger emergency`,`error`)}catch(e){console.error(`Error triggering emergency:`,e),y(`Failed to trigger emergency`,`error`)}}async function zn(){try{let e=m();(await fetch(`${e}/api/emergency/test`,{method:`GET`,headers:h()})).ok?y(`Test notification sent successfully`,`success`):y(`Failed to send test notification`,`error`)}catch(e){console.error(`Error testing emergency notification:`,e),y(`Failed to send test notification`,`error`)}}window.loadEmergencyContacts=Dn,window.loadEmergencyHistory=kn,window.deleteEmergencyContact=An,window.loadE911Sites=jn,window.loadExtensionLocations=Mn,window.showAddEmergencyContactModal=Nn,window.closeAddEmergencyContactModal=Pn,window.showTriggerEmergencyModal=Fn,window.closeTriggerEmergencyModal=In,window.addEmergencyContact=Ln,window.triggerEmergency=Rn,window.testEmergencyNotification=zn;async function Bn(){try{let e=m(),t=await fetch(`${e}/api/phone-book`,{headers:h()});if(!t.ok)throw Error(`HTTP ${t.status}`);let n=await t.json(),r=document.getElementById(`phone-book-body`);if(!r)return;let i=n.entries??[];if(i.length===0){r.innerHTML=`<tr><td colspan="5">No phone book entries</td></tr>`;return}r.innerHTML=i.map(e=>`
            <tr>
                <td>${_(e.name||``)}</td>
                <td>${_(e.number||``)}</td>
                <td>${_(e.email||``)}</td>
        ...
        `).join(``)}catch(e){console.error(`Error loading phone book:`,e)}}async function Vn(e){if(confirm(`Delete this phone book entry?`))try{let t=m();(await fetch(`${t}/api/phone-book/${e}`,{method:`DELETE`,headers:h()})).ok&&(y(`Phone book entry deleted`,`success`),Bn())}catch(e){console.error(`Error deleting entry:`,e),y(`Failed to delete entry`,`error`)}}window.loadPhoneBook=Bn,window.deletePhoneBookEntry=Vn;var Hn=[],E=``;async function D(e,t){try{let n=await e.json();return n.error??n.message??t}catch{return t}}async function Un(){await Wn(),await Promise.all([O(),k(),Gn()])}async function Wn(){let e=document.getElementById(`paging-disabled-banner`),t=[`add-paging-zone-btn`,`add-paging-device-btn`,`test-page-submit`].map(e=>document.getElementById(e));try{let n=await fetch(`${m()}/api/paging/status`,{headers:h()});if(!n.ok)throw Error(`HTTP ${n.status}`);let r=await n.json();if(e&&(e.style.display=r.enabled?`none`:`block`),t.forEach(e=>{e&&(e.disabled=!r.enabled)}),r.all_call_extension){E=r.all_call_extension;let e=document.getElementById(`paging-all-call-ext`);e&&(e.textContent=r.all_call_extension)}}catch(e){console.error(`Error loading paging status:`,e)}}async function O(){let e=document.getElementById(`paging-zones-table-body`);if(e)try{let t=await fetch(`${m()}/api/paging/zones`,{headers:h()});if(!t.ok)throw Error(`HTTP ${t.status}`);let n=(await t.json()).zones??[];n.length===0?e.innerHTML=`<tr><td colspan="5">No paging zones configured</td></tr>`:e.innerHTML=n.map(e=>`
                <tr>
                    <td>${_(e.extension??``)}</td>
                    <td>${_(e.name??``)}</td>
                    <td>${_(e.description??``)}</td>
                    <td>${e.dac_device?_(e.dac_device):`<em>none — answers but plays no audio</em>`}</td>
                    <td><button class="btn btn-danger btn-sm"
                        onclick="deletePagingZone('${_(e.extension)}')">Delete</button></td>
                </tr>
            `).join(``),Kn(n)}catch(t){console.error(`Error loading paging zones:`,t),e.innerHTML=`<tr><td colspan="5">Failed to load paging zones</td></tr>`}}async function k(){let e=document.getElementById(`paging-devices-table-body`);if(e)try{let t=await fetch(`${m()}/api/paging/devices`,{headers:h()});if(!t.ok)throw Error(`HTTP ${t.status}`);let n=(await t.json()).devices??[];Hn=n,n.length===0?e.innerHTML=`<tr><td colspan="6">No DAC devices configured</td></tr>`:e.innerHTML=n.map(e=>{let t=e.sip_uri??(e.ip_address?`${e.ip_address}:${e.port??5060}`:``);return`
                <tr>
                    <td>${_(e.device_id)}</td>
                    <td>${_(e.name??e.device_id)}</td>
                    <td>${_(e.device_type??``)}</td>
                    <td>${_(t)}</td>
                    <td>${_(e.configured_at?Ke(e.configured_at):``)}</td>
                    <td><button class="btn btn-danger btn-sm"
                        onclick="deletePagingDevice('${_(e.device_id)}')">Delete</button></td>
                </tr>`}).join(``),qn(n)}catch(t){console.error(`Error loading paging devices:`,t),e.innerHTML=`<tr><td colspan="6">Failed to load DAC devices</td></tr>`}}async function Gn(){let e=document.getElementById(`active-pages-table-body`);if(e)try{let t=await fetch(`${m()}/api/paging/active`,{headers:h()});if(!t.ok)throw Error(`HTTP ${t.status}`);let n=(await t.json()).active_pages??[];e.innerHTML=n.length===0?`<tr><td colspan="5">No active paging sessions</td></tr>`:n.map(e=>`
                <tr>
                    <td>${_(e.page_id)}</td>
                    <td>${_(e.from_extension??``)}</td>
                    <td>${_(e.zone_names??``)}</td>
                    <td>${_(e.started_at?Ke(e.started_at):``)}</td>
                    <td>${_(e.status??``)}</td>
                </tr>
            `).join(``)}catch(t){console.error(`Error loading active pages:`,t),e.innerHTML=`<tr><td colspan="5">Failed to load active pages</td></tr>`}}function Kn(e){let t=document.getElementById(`test-page-zone`);if(!t)return;let n=t.value;t.innerHTML=`<option value="">Select Zone</option>`+(E?`<option value="${_(E)}">${_(E)} — All Zones</option>`:``)+e.map(e=>`<option value="${_(e.extension)}">${_(e.extension)} — ${_(e.name??``)}</option>`).join(``),n&&(t.value=n)}function qn(e){let t=document.getElementById(`paging-zone-dac-device`);t&&(t.innerHTML=`<option value="">None (zone answers but plays no audio)</option>`+e.map(e=>`<option value="${_(e.device_id)}">${_(e.name??e.device_id)}</option>`).join(``))}function Jn(){qn(Hn);let e=document.getElementById(`add-paging-zone-modal`);e&&e.classList.add(`active`)}function Yn(){let e=document.getElementById(`add-paging-zone-modal`);e&&e.classList.remove(`active`),document.getElementById(`add-paging-zone-form`)?.reset()}function Xn(){let e=document.getElementById(`add-paging-device-modal`);e&&e.classList.add(`active`)}function Zn(){let e=document.getElementById(`add-paging-device-modal`);e&&e.classList.remove(`active`),document.getElementById(`add-paging-device-form`)?.reset()}var A=e=>(document.getElementById(e)?.value??``).trim();async function Qn(e){e.preventDefault();let t={extension:A(`paging-zone-extension`),name:A(`paging-zone-name`),description:A(`paging-zone-description`),dac_device:A(`paging-zone-dac-device`)||null};try{let e=await fetch(`${m()}/api/paging/zones`,{method:`POST`,headers:{...h(),"Content-Type":`application/json`},body:JSON.stringify(t)});e.ok?(y(`Zone ${t.name} added`,`success`),Yn(),O()):y(await D(e,`Failed to add zone`),`error`)}catch(e){console.error(`Error adding paging zone:`,e),y(`Failed to add zone`,`error`)}}async function $n(e){e.preventDefault();let t=A(`paging-device-port`),n={device_id:A(`paging-device-id`),name:A(`paging-device-name`)||void 0,device_type:A(`paging-device-type`),ip_address:A(`paging-device-ip`),sip_uri:A(`paging-device-sip-uri`),port:t?Number(t):5060};try{let e=await fetch(`${m()}/api/paging/devices`,{method:`POST`,headers:{...h(),"Content-Type":`application/json`},body:JSON.stringify(n)});e.ok?(y(`Device ${n.device_id} added`,`success`),Zn(),k()):y(await D(e,`Failed to add device`),`error`)}catch(e){console.error(`Error adding paging device:`,e),y(`Failed to add device`,`error`)}}async function er(e){if(confirm(`Delete paging zone ${e}?`))try{let t=await fetch(`${m()}/api/paging/zones/${encodeURIComponent(e)}`,{method:`DELETE`,headers:h()});t.ok?(y(`Zone ${e} deleted`,`success`),O()):y(await D(t,`Failed to delete zone`),`error`)}catch(e){console.error(`Error deleting paging zone:`,e),y(`Failed to delete zone`,`error`)}}async function tr(e){if(confirm(`Delete paging device ${e}?`))try{let t=await fetch(`${m()}/api/paging/devices/${encodeURIComponent(e)}`,{method:`DELETE`,headers:h()});t.ok?(y(`Device ${e} deleted`,`success`),k(),O()):y(await D(t,`Failed to delete device`),`error`)}catch(e){console.error(`Error deleting paging device:`,e),y(`Failed to delete device`,`error`)}}async function nr(e){e.preventDefault();let t={from_extension:A(`test-page-from`),zone:A(`test-page-zone`)};try{let e=await fetch(`${m()}/api/paging/test`,{method:`POST`,headers:{...h(),"Content-Type":`application/json`},body:JSON.stringify(t)});e.ok?(y(`Ringing ${t.from_extension} — answer to page zone ${t.zone}`,`success`),Gn()):y(await D(e,`Failed to start test page`),`error`)}catch(e){console.error(`Error starting test page:`,e),y(`Failed to start test page`,`error`)}}window.loadPagingData=Un,window.loadPagingZones=O,window.loadPagingDevices=k,window.loadActivePages=Gn,window.deletePagingZone=er,window.deletePagingDevice=tr,window.loadPagingStatus=Wn,window.showAddPagingZoneModal=Jn,window.closeAddPagingZoneModal=Yn,window.showAddPagingDeviceModal=Xn,window.closeAddPagingDeviceModal=Zn,window.addPagingZone=Qn,window.addPagingDevice=$n,window.startTestPage=nr;async function j(){let e=document.getElementById(`license-status-container`);if(e){e.innerHTML=`<div class="loading">Loading license information...</div>`;try{let t=await g(`${m()}/api/license/status`,{headers:h()});if(!t.ok)throw Error(`HTTP ${t.status}`);let n=await t.json();if(n.success&&n.license){let t=n.license,r=t.status===`active`||t.valid?`badge-success`:`badge-danger`,i=t.status||(t.valid?`Active`:`Invalid`);e.innerHTML=`
            ...
            `}else e.innerHTML=`<div class="info-box">No license installed. System is running in open-source mode with all features available.</div>`}catch(t){console.error(`Error loading license status:`,t),e.innerHTML=`<div class="error-message">Failed to load license status</div>`}}}async function M(){let e=document.getElementById(`license-features-container`);if(e){e.innerHTML=`<div class="loading">Loading features...</div>`;try{let t=await g(`${m()}/api/license/features`,{headers:h()});if(!t.ok)throw Error(`HTTP ${t.status}`);let n=await t.json();if(!n.licensing_enabled){e.innerHTML=`<div class="info-box">Licensing disabled &mdash; all features are available (open-source mode).</div>`;return}let r=n.features??[],i=n.limits??{},a=`<p style="margin-bottom: 10px;"><strong>License Type:</strong> ${(n.license_type||`unknown`).toUpperCase()}</p>`;if(Object.keys(i).length>0){a+=`<h4 style="margin: 15px 0 8px;">Limits</h4><div class="ad-status-grid">`;for(let[e,t]of Object.entries(i))a+=`<div class="ad-status-item"><strong>${e.replace(/_/g,` `)}</strong><span>${t??`Unlimited`}</span></div>`;a+=`</div>`}if(r.length>0){a+=`<h4 style="margin: 15px 0 8px;">Included Features</h4><div style="display: flex; flex-wrap: wrap; gap: 8px;">`;for(let e of r)a+=`<span class="badge badge-success" style="padding: 4px 10px;">${e.replace(/_/g,` `)}</span>`;a+=`</div>`}e.innerHTML=a}catch(t){console.error(`Error loading license features:`,t),e.innerHTML=`<div class="error-message">Failed to load license features</div>`}}}async function rr(e){e&&e.preventDefault();let t=document.getElementById(`generate-license-result`),n=document.getElementById(`license-type`)?.value,r=document.getElementById(`issued-to`)?.value?.trim(),i=document.getElementById(`expiration-days`)?.value,a=document.getElementById(`max-extensions`)?.value,o=document.getElementById(`max-concurrent-calls`)?.value;if(!n||!r){y(`License type and organization/person are required`,`error`);return}let s={type:n,issued_to:r};i&&(s.expiration_days=parseInt(i,10)),a&&(s.max_extensions=parseInt(a,10)),o&&(s.max_concurrent_calls=parseInt(o,10));try{let e=await(await g(`${m()}/api/license/generate`,{method:`POST`,headers:h(),body:JSON.stringify(s)})).json();if(e.success&&e.license){y(`License generated successfully`,`success`);let n=JSON.stringify(e.license,null,2);t&&(t.innerHTML=`
                ...
                `)}else y(e.error||`Failed to generate license`,`error`),t&&(t.innerHTML=`<div class="error-message">${e.error||`Failed to generate license`}</div>`)}catch(e){console.error(`Error generating license:`,e),y(`Failed to generate license`,`error`),t&&(t.innerHTML=`<div class="error-message">Failed to generate license</div>`)}}async function ir(){let e=document.getElementById(`generated-license-json`);if(e)try{await or(JSON.parse(e.textContent||`{}`),!1)}catch(e){console.error(`Error auto-installing license:`,e),y(`Failed to install generated license`,`error`)}}async function ar(e){e&&e.preventDefault();let t=document.getElementById(`license-data`),n=document.getElementById(`enforce-licensing`),r=document.getElementById(`install-license-result`);if(!t||!t.value.trim()){y(`Please enter license data (JSON)`,`error`);return}let i;try{i=JSON.parse(t.value.trim())}catch{y(`Invalid JSON format. Please check the license data.`,`error`);return}let a=n?.checked??!1;try{await or(i,a),t.value=``,n&&(n.checked=!1)}catch(e){console.error(`Error installing license:`,e),y(`Failed to install license`,`error`),r&&(r.innerHTML=`<div class="error-message">Failed to install license</div>`)}}async function or(e,t){let n=document.getElementById(`install-license-result`),r=await(await g(`${m()}/api/license/install`,{method:`POST`,headers:h(),body:JSON.stringify({license_data:e,enforce_licensing:t})})).json();r.success?(y(r.message||`License installed successfully`,`success`),n&&(n.innerHTML=`<div class="info-box" style="border-left-color: #4caf50;">${r.message||`License installed successfully`}</div>`),j(),M()):(y(r.error||`Failed to install license`,`error`),n&&(n.innerHTML=`<div class="error-message">${r.error||`Failed to install license`}</div>`))}async function sr(e){let t=document.getElementById(`licensing-toggle-result`),n=e?`enable`:`disable`;if(confirm(`Are you sure you want to ${n} licensing?`))try{let r=await(await g(`${m()}/api/license/toggle`,{method:`POST`,headers:h(),body:JSON.stringify({enabled:e})})).json();r.success?(y(r.message||`Licensing ${n}d successfully`,`success`),t&&(t.innerHTML=`<div class="info-box" style="border-left-color: #4caf50;">${r.message||`Licensing ${n}d successfully`}</div>`),j(),M()):(y(r.error||`Failed to ${n} licensing`,`error`),t&&(t.innerHTML=`<div class="error-message">${r.error||`Failed to ${n} licensing`}</div>`))}catch(e){console.error(`Error toggling licensing:`,e),y(`Failed to ${n} licensing`,`error`),t&&(t.innerHTML=`<div class="error-message">Failed to ${n} licensing</div>`)}}async function cr(){if(!confirm(`Are you sure you want to revoke the current license? This action cannot be undone.`))return;let e=document.getElementById(`revoke-license-result`);try{let t=await(await g(`${m()}/api/license/revoke`,{method:`POST`,headers:h()})).json();t.success?(y(`License revoked successfully`,`success`),e&&(e.innerHTML=`<div class="info-box" style="border-left-color: #4caf50;">License revoked successfully</div>`),j(),M()):(y(t.error||`Failed to revoke license`,`error`),e&&(e.innerHTML=`<div class="error-message">${t.error||`Failed to revoke license`}</div>`))}catch(t){console.error(`Error revoking license:`,t),y(`Failed to revoke license`,`error`),e&&(e.innerHTML=`<div class="error-message">Failed to revoke license</div>`)}}async function lr(){if(!confirm(`Are you sure you want to remove the license lock? This will allow licensing to be disabled.`))return;let e=document.getElementById(`remove-lock-result`);try{let t=await(await g(`${m()}/api/license/remove_lock`,{method:`POST`,headers:h()})).json();t.success?(y(t.message||`License lock removed`,`success`),e&&(e.innerHTML=`<div class="info-box" style="border-left-color: #4caf50;">${t.message||`License lock removed`}</div>`)):(y(t.error||`Failed to remove license lock`,`error`),e&&(e.innerHTML=`<div class="error-message">${t.error||`Failed to remove license lock`}</div>`))}catch(t){console.error(`Error removing license lock:`,t),y(`Failed to remove license lock`,`error`),e&&(e.innerHTML=`<div class="error-message">Failed to remove license lock</div>`)}}function ur(){j(),M()}window.loadLicenseStatus=j,window.loadLicenseFeatures=M,window.generateLicense=rr,window.autoInstallGeneratedLicense=ir,window.installLicense=ar,window.toggleLicensing=sr,window.revokeLicense=cr,window.removeLicenseLock=lr,window.initLicenseManagement=ur;var N={};function dr(){return typeof Chart<`u`}async function fr(){try{let e=document.getElementById(`analytics-period`)?.value??`7`,t=m(),n=await fetch(`${t}/api/analytics/overview?days=${e}`,{headers:h()});if(!n.ok)throw Error(`HTTP ${n.status}`);let r=await n.json();pr(r),mr(r.top_callers??[]),dr()&&(r.daily_trends&&hr(r.daily_trends),r.hourly_distribution&&gr(r.hourly_distribution),r.disposition&&_r(r.disposition))}catch(e){console.error(`Error loading analytics:`,e)}}function pr(e){let t=e=>document.getElementById(e),n=t(`analytics-total-calls`),r=t(`analytics-avg-duration`),i=t(`analytics-answer-rate`),a=t(`analytics-answered-calls`);n&&(n.textContent=String(e.total_calls??0)),r&&(r.textContent=`${e.avg_duration??0}s`),i&&(i.textContent=`${e.answer_rate??0}%`),a&&(a.textContent=String(e.answered_calls??0))}function mr(e){let t=document.getElementById(`top-callers-table`);if(t){if(e.length===0){t.innerHTML=`<tr><td colspan="4" class="loading">No call data available</td></tr>`;return}t.innerHTML=e.map(e=>`<tr>
            <td>${_(String(e.extension??`Unknown`))}</td>
            <td>${e.calls??0}</td>
            <td>${((e.total_duration??0)/60).toFixed(1)}</td>
            <td>${(e.avg_duration??0).toFixed(1)}</td>
        </tr>`).join(``)}}function hr(e){let t=document.getElementById(`daily-trends-chart`)?.getContext(`2d`);t&&(N.dailyTrends&&N.dailyTrends.destroy(),N.dailyTrends=new Chart(t,{type:`line`,data:{labels:e.labels||[],datasets:[{label:`Calls`,data:e.data||[],borderColor:`#3b82f6`,tension:.3,fill:!1}]},options:{responsive:!0,maintainAspectRatio:!1}}))}function gr(e){let t=document.getElementById(`hourly-distribution-chart`)?.getContext(`2d`);t&&(N.hourlyDist&&N.hourlyDist.destroy(),N.hourlyDist=new Chart(t,{type:`bar`,data:{labels:e.labels||[],datasets:[{label:`Calls by Hour`,data:e.data||[],backgroundColor:`#60a5fa`}]},options:{responsive:!0,maintainAspectRatio:!1}}))}function _r(e){let t=document.getElementById(`disposition-chart`)?.getContext(`2d`);t&&(N.disposition&&N.disposition.destroy(),N.disposition=new Chart(t,{type:`doughnut`,data:{labels:e.labels||[],datasets:[{data:e.data||[],backgroundColor:[`#10b981`,`#ef4444`,`#f59e0b`,`#6b7280`]}]},options:{responsive:!0,maintainAspectRatio:!1}}))}async function vr(){try{let e=m(),t=await fetch(`${e}/api/qos/metrics`,{headers:h()});if(!t.ok)throw Error(`HTTP ${t.status}`);let n=await t.json(),r=n.active_calls??0,i=n.metrics??[],a=0,o=0;if(i.length>0){let e=i.map(e=>e.mos_score??0).filter(e=>e>0);a=e.length>0?e.reduce((e,t)=>e+t,0)/e.length:0,o=e.filter(e=>e<3.5).length}let s=document.getElementById(`qos-avg-mos`),c=document.getElementById(`qos-active-calls`),l=document.getElementById(`qos-total-calls`),u=document.getElementById(`qos-calls-with-issues`);s&&(s.textContent=a>0?a.toFixed(2):`N/A`),c&&(c.textContent=String(r)),l&&(l.textContent=String(i.length)),u&&(u.textContent=String(o))}catch(e){console.error(`Error loading QoS metrics:`,e)}}async function yr(){try{let e=m();if((await fetch(`${e}/api/qos/clear-alerts`,{method:`POST`,headers:h()})).ok){let e=document.getElementById(`qos-alerts-container`);e&&(e.innerHTML=`<div class="info-box">No quality alerts</div>`),y(`QoS alerts cleared`,`success`)}else y(`Failed to clear QoS alerts`,`error`)}catch(e){console.error(`Error clearing QoS alerts:`,e),y(`Failed to clear QoS alerts`,`error`)}}async function br(e){e&&e.preventDefault();try{let e=e=>document.getElementById(e)?.value??``,t={mos_min:parseFloat(e(`qos-threshold-mos`))||3.5,jitter_max:parseInt(e(`qos-threshold-jitter`),10)||50,packet_loss_max:parseFloat(e(`qos-threshold-loss`))||2,latency_max:parseInt(e(`qos-threshold-latency`),10)||300},n=m();(await fetch(`${n}/api/qos/thresholds`,{method:`POST`,headers:{...h(),"Content-Type":`application/json`},body:JSON.stringify(t)})).ok?y(`QoS thresholds saved`,`success`):y(`Failed to save QoS thresholds`,`error`)}catch(e){console.error(`Error saving QoS thresholds:`,e),y(`Failed to save QoS thresholds`,`error`)}}window.loadAnalytics=fr,window.loadQoSMetrics=vr,window.clearQoSAlerts=yr,window.saveQoSThresholds=br;function xr(e){return{registered:`<span class="badge" style="background: #10b981;">Registered</span>`,unregistered:`<span class="badge" style="background: #6b7280;">Unregistered</span>`,failed:`<span class="badge" style="background: #ef4444;">Failed</span>`,disabled:`<span class="badge" style="background: #9ca3af;">Disabled</span>`,degraded:`<span class="badge" style="background: #f59e0b;">Degraded</span>`}[e]||e}function Sr(e){return{healthy:`<span class="badge" style="background: #10b981;">Healthy</span>`,warning:`<span class="badge" style="background: #f59e0b;">Warning</span>`,critical:`<span class="badge" style="background: #f59e0b;">Critical</span>`,down:`<span class="badge" style="background: #ef4444;">Down</span>`}[e]||e}var Cr={0:`PCMU (G.711u)`,8:`PCMA (G.711a)`,9:`G.722`,18:`G.729`,2:`G.726-32`};function wr(e){return!e||e.length===0?`—`:e.map(e=>Cr[e]||e).join(`, `)}async function P(){try{let e=await g(`${m()}/api/sip-trunks`,{headers:h()});if(!e.ok)throw Error(`HTTP ${e.status}: ${e.statusText}`);let t=await e.json();if(t.trunks){let e=document.getElementById(`trunk-total`);e&&(e.textContent=String(t.count||0));let n=t.trunks.filter(e=>e.health_status===`healthy`).length,r=t.trunks.filter(e=>e.status===`registered`).length,i=t.trunks.reduce((e,t)=>e+t.channels_available,0),a=document.getElementById(`trunk-healthy`);a&&(a.textContent=String(n));let o=document.getElementById(`trunk-registered`);o&&(o.textContent=String(r));let s=document.getElementById(`trunk-total-channels`);s&&(s.textContent=String(i));let c=document.getElementById(`trunks-list`);if(!c)return;t.trunks.length===0?c.innerHTML=`<tr><td colspan="9" style="text-align: center;">No SIP trunks configured</td></tr>`:c.innerHTML=t.trunks.map(e=>{let t=xr(e.status),n=Sr(e.health_status),r=(e.success_rate*100).toFixed(1);return`
                        <tr>
                            <td><strong>${_(e.name)}</strong><br/><small>${_(e.trunk_id)}</small></td>
                            <td>${_(e.host)}:${e.port}</td>
                            <td>${_(wr(e.codec_preferences))}</td>
                            <td>${t}</td>
                            <td>${n}</td>
                            <td>${e.priority}</td>
            ...
            `).join(``),y(`Health metrics loaded`,`success`)}}catch(e){console.error(`Error loading trunk health:`,e),y(`Error loading trunk health: ${e instanceof Error?e.message:String(e)}`,`error`)}}function Er(){let e=document.getElementById(`add-trunk-modal`);e&&(e.style.display=`block`)}function Dr(){let e=document.getElementById(`add-trunk-modal`);e&&(e.style.display=`none`);let t=document.getElementById(`add-trunk-form`);t&&t.reset()}async function Or(e){e.preventDefault();let t=Array.from(document.querySelectorAll(`input[name="trunk-codecs"]:checked`)).flatMap(e=>e.value.split(`,`)),n={trunk_id:document.getElementById(`trunk-id`).value,name:document.getElementById(`trunk-name`).value,host:document.getElementById(`trunk-host`).value,port:parseInt(document.getElementById(`trunk-port`).value),username:document.getElementById(`trunk-username`).value,password:document.getElementById(`trunk-password`).value,priority:parseInt(document.getElementById(`trunk-priority`).value),max_channels:parseInt(document.getElementById(`trunk-channels`).value),codec_preferences:t.length>0?t:[`0`,`8`,`18`]};try{let e=await(await g(`${m()}/api/sip-trunks`,{method:`POST`,headers:{...h(),"Content-Type":`application/json`},body:JSON.stringify(n)})).json();e.success?(y(`Trunk ${n.name} added successfully`,`success`),Dr(),P()):y(e.error||`Error adding trunk`,`error`)}catch(e){console.error(`Error adding trunk:`,e),y(`Error adding trunk`,`error`)}}async function kr(e,t){if(confirm(`Are you sure you want to delete trunk "${t}"?`))try{let n=await(await g(`${m()}/api/sip-trunks/${e}`,{method:`DELETE`,headers:h()})).json();n.success?(y(`Trunk ${t} deleted`,`success`),P()):y(n.error||`Error deleting trunk`,`error`)}catch(e){console.error(`Error deleting trunk:`,e),y(`Error deleting trunk`,`error`)}}async function Ar(e){y(`Testing trunk...`,`info`);try{let t=await(await g(`${m()}/api/sip-trunks/test`,{method:`POST`,headers:{...h(),"Content-Type":`application/json`},body:JSON.stringify({trunk_id:e})})).json();if(t.success){let e=t.health_status??`unknown`;y(`Trunk test complete: ${e}`,e===`healthy`?`success`:`warning`),P(),Tr()}else y(t.error||`Error testing trunk`,`error`)}catch(e){console.error(`Error testing trunk:`,e),y(`Error testing trunk`,`error`)}}var F=[];function jr(e){if(e.source===`manual`)return`Manual`;let t=e.shadowed?` <span class="badge" style="background:#6b7280;">shadowed by a manual route</span>`:``;return`via extension ${_(e.destination_value)}${t}`}async function Mr(){try{let e=await g(`${m()}/api/inbound-routes`,{headers:h()});if(!e.ok)throw Error(`HTTP ${e.status}: ${e.statusText}`);F=(await e.json()).routes??[];let t=document.getElementById(`inbound-routes-list`);if(!t)return;if(F.length===0){t.innerHTML=`<tr><td colspan="7" style="text-align: center;">No inbound routes configured</td></tr>`;return}t.innerHTML=F.map(e=>{let t=e.source===`manual`&&e.id!==void 0?`
            ...
            `}).join(``)}catch(e){console.error(`Error loading inbound routes:`,e),y(`Error loading inbound routes: ${e instanceof Error?e.message:String(e)}`,`error`)}}function Nr(){let e=document.getElementById(`inbound-route-form`);e&&e.reset();let t=document.getElementById(`inbound-route-id`);t&&(t.value=``);let n=document.getElementById(`inbound-route-modal-title`);n&&(n.textContent=`➕ Add Inbound Route`);let r=document.getElementById(`inbound-route-modal`);r&&(r.style.display=`block`)}function Pr(e){let t=F.find(t=>t.id===e);if(!t)return;let n=document.getElementById(`inbound-route-id`);n&&(n.value=String(e));let r=document.getElementById(`inbound-route-did`);r&&(r.value=t.did_number);let i=document.getElementById(`inbound-route-trunk`);i&&(i.value=t.trunk_id??``);let a=document.getElementById(`inbound-route-destination-type`);a&&(a.value=t.destination_type);let o=document.getElementById(`inbound-route-destination-value`);o&&(o.value=t.destination_value);let s=document.getElementById(`inbound-route-priority`);s&&(s.value=String(t.priority));let c=document.getElementById(`inbound-route-enabled`);c&&(c.checked=t.enabled);let l=document.getElementById(`inbound-route-modal-title`);l&&(l.textContent=`✏️ Edit Inbound Route (${t.did_number})`);let u=document.getElementById(`inbound-route-modal`);u&&(u.style.display=`block`)}function Fr(){let e=document.getElementById(`inbound-route-modal`);e&&(e.style.display=`none`);let t=document.getElementById(`inbound-route-form`);t&&t.reset()}async function Ir(e){e.preventDefault();let t=e=>document.getElementById(e)?.value??``,n=t(`inbound-route-id`),r={did_number:t(`inbound-route-did`),trunk_id:t(`inbound-route-trunk`),destination_type:t(`inbound-route-destination-type`),destination_value:t(`inbound-route-destination-value`),priority:parseInt(t(`inbound-route-priority`))||100,enabled:document.getElementById(`inbound-route-enabled`)?.checked??!0};try{let e=m(),t=await(await g(n?`${e}/api/inbound-routes/${n}`:`${e}/api/inbound-routes`,{method:n?`PUT`:`POST`,headers:{...h(),"Content-Type":`application/json`},body:JSON.stringify(r)})).json();t.success?(y(`Inbound route for ${r.did_number} saved`,`success`),Fr(),Mr()):y(t.error||`Error saving inbound route`,`error`)}catch(e){console.error(`Error saving inbound route:`,e),y(`Error saving inbound route`,`error`)}}async function Lr(e,t){if(confirm(`Are you sure you want to delete the inbound route for "${t}"?`))try{let n=await(await g(`${m()}/api/inbound-routes/${e}`,{method:`DELETE`,headers:h()})).json();n.success?(y(`Inbound route for ${t} deleted`,`success`),Mr()):y(n.error||`Error deleting inbound route`,`error`)}catch(e){console.error(`Error deleting inbound route:`,e),y(`Error deleting inbound route`,`error`)}}async function I(){try{let e=await g(`${m()}/api/lcr/rates`,{headers:h()});if(!e.ok){window.suppressErrorNotifications?debugLog(`LCR rates endpoint returned error:`,e.status,`(feature may not be enabled)`):(console.error(`Error loading LCR rates:`,e.status),y(`Error loading LCR rates`,`error`));return}let t=await e.json();if(t.rates!==void 0){let e=document.getElementById(`lcr-total-rates`);e&&(e.textContent=String(t.count||0));let n=document.getElementById(`lcr-time-rates`);n&&(n.textContent=String(t.time_rates?t.time_rates.length:0));let r=document.getElementById(`lcr-rates-list`);r&&(t.rates.length===0?r.innerHTML=`<tr><td colspan="7" style="text-align: center;">No rates configured</td></tr>`:r.innerHTML=t.rates.map(e=>`
                        <tr>
                    ...
                    `).join(``))}catch(e){if(window.suppressErrorNotifications){let t=e instanceof Error?e.message:String(e);debugLog(`Error loading LCR statistics (expected if LCR not enabled):`,t)}else console.error(`Error loading LCR statistics:`,e)}}function zr(){document.body.insertAdjacentHTML(`beforeend`,`
    ...
    `)}function Br(){let e=document.getElementById(`lcr-rate-modal`);e&&e.remove()}function Vr(e){if(!e||e.trim().length===0)return{valid:!1,error:`Pattern cannot be empty`};try{return new RegExp(e),{valid:!0}}catch(e){return{valid:!1,error:`Invalid regex: ${e instanceof Error?e.message:`Invalid regex pattern`}`}}}async function Hr(e){e.preventDefault();let t=document.getElementById(`lcr-pattern`).value,n=Vr(t);if(!n.valid){y(n.error||`Invalid LCR pattern`,`error`);return}let r={trunk_id:document.getElementById(`lcr-trunk-id`).value,pattern:t,description:document.getElementById(`lcr-description`).value,rate_per_minute:parseFloat(document.getElementById(`lcr-rate-per-minute`).value),connection_fee:parseFloat(document.getElementById(`lcr-connection-fee`).value),minimum_seconds:parseInt(document.getElementById(`lcr-minimum-seconds`).value),billing_increment:parseInt(document.getElementById(`lcr-billing-increment`).value)};try{let e=await(await g(`${m()}/api/lcr/rate`,{method:`POST`,headers:{...h(),"Content-Type":`application/json`},body:JSON.stringify(r)})).json();e.success?(y(`LCR rate added successfully`,`success`),Br(),I()):y(e.error||`Error adding LCR rate`,`error`)}catch(e){console.error(`Error adding LCR rate:`,e),y(`Error adding LCR rate: ${e instanceof Error?e.message:`Unknown error`}`,`error`)}}function Ur(){document.body.insertAdjacentHTML(`beforeend`,`
    ...
    `)}function Wr(){let e=document.getElementById(`lcr-time-rate-modal`);e&&e.remove()}async function Gr(e){e.preventDefault();let t=Array.from(document.querySelectorAll(`input[name="time-days"]:checked`)).map(e=>parseInt(e.value)),n={name:document.getElementById(`time-rate-name`).value,start_hour:parseInt(document.getElementById(`time-rate-start-hour`).value),start_minute:parseInt(document.getElementById(`time-rate-start-minute`).value),end_hour:parseInt(document.getElementById(`time-rate-end-hour`).value),end_minute:parseInt(document.getElementById(`time-rate-end-minute`).value),days:t,multiplier:parseFloat(document.getElementById(`time-rate-multiplier`).value)};try{let e=await(await g(`${m()}/api/lcr/time-rate`,{method:`POST`,headers:{...h(),"Content-Type":`application/json`},body:JSON.stringify(n)})).json();e.success?(y(`Time-based rate added successfully`,`success`),Wr(),I()):y(e.error||`Error adding time-based rate`,`error`)}catch(e){console.error(`Error adding time-based rate:`,e),y(`Error adding time-based rate`,`error`)}}async function Kr(){if(confirm(`Are you sure you want to clear all LCR rates? This cannot be undone.`))try{let e=await(await g(`${m()}/api/lcr/clear-rates`,{method:`POST`,headers:{...h(),"Content-Type":`application/json`},body:JSON.stringify({})})).json();e.success?(y(`All LCR rates cleared`,`success`),I()):y(e.error||`Error clearing LCR rates`,`error`)}catch(e){console.error(`Error clearing LCR rates:`,e),y(`Error clearing LCR rates`,`error`)}}window.loadSIPTrunks=P,window.loadTrunkHealth=Tr,window.showAddTrunkModal=Er,window.closeAddTrunkModal=Dr,window.addSIPTrunk=Or,window.deleteTrunk=kr,window.testTrunk=Ar,window.loadInboundRoutes=Mr,window.showAddInboundRouteModal=Nr,window.editInboundRoute=Pr,window.closeInboundRouteModal=Fr,window.saveInboundRoute=Ir,window.deleteInboundRoute=Lr,window.loadLCRRates=I,window.loadLCRStatistics=Rr,window.showAddLCRRateModal=zr,window.closeLCRRateModal=Br,window.addLCRRate=Hr,window.showAddTimeRateModal=Ur,window.closeTimeRateModal=Wr,window.addTimeRate=Gr,window.clearLCRRates=Kr;var qr=0;async function Jr(){try{let e=await g(`${m()}/api/fmfm/extensions`,{headers:h()});if(!e.ok)throw Error(`HTTP ${e.status}: ${e.statusText}`);let t=await e.json();if(t.extensions){let e=document.getElementById(`fmfm-total-extensions`);e&&(e.textContent=String(t.count||0));let n=t.extensions.filter(e=>e.mode===`sequential`).length,r=t.extensions.filter(e=>e.mode===`simultaneous`).length,i=t.extensions.filter(e=>e.enabled!==!1).length,a=document.getElementById(`fmfm-sequential`);a&&(a.textContent=String(n));let o=document.getElementById(`fmfm-simultaneous`);o&&(o.textContent=String(r));let s=document.getElementById(`fmfm-active-count`);s&&(s.textContent=String(i));let c=document.getElementById(`fmfm-list`);if(!c)return;t.extensions.length===0?c.innerHTML=`<tr><td colspan="6" style="text-align: center;">No Find Me/Follow Me configurations</td></tr>`:c.innerHTML=t.extensions.map(e=>{let t=e.enabled!==!1,n=e.mode===`sequential`?`<span class="badge" style="background: #3b82f6;">Sequential</span>`:`<span class="badge" style="background: #10b981;">Simultaneous</span>`,r=t?`<span class="badge" style="background: #10b981;">Active</span>`:`<span class="badge" style="background: #6b7280;">Disabled</span>`,i=e.destinations||[],a=i.map(e=>`${_(e.number)}${e.ring_time?` (${e.ring_time}s)`:``}`).join(`, `),o=e.updated_at?new Date(e.updated_at).toLocaleString():`N/A`;return`
                        <tr>
                    ...
                    `}).join(``)}}catch(e){console.error(`Error loading FMFM extensions:`,e),y(`Error loading FMFM configurations`,`error`)}}function Yr(){let e=document.getElementById(`add-fmfm-modal`);e&&(e.style.display=`block`);let t=document.getElementById(`fmfm-extension`);t&&(t.value=``,t.readOnly=!1);let n=document.getElementById(`fmfm-mode`);n&&(n.value=`sequential`);let r=document.getElementById(`fmfm-enabled`);r&&(r.checked=!0);let i=document.getElementById(`fmfm-no-answer`);i&&(i.value=``);let a=document.getElementById(`fmfm-destinations-list`);a&&(a.innerHTML=``),L()}function Xr(){let e=document.getElementById(`add-fmfm-modal`);e&&(e.style.display=`none`);let t=document.getElementById(`add-fmfm-form`);t&&t.reset()}function L(){let e=document.getElementById(`fmfm-destinations-list`);if(!e)return;let t=`fmfm-dest-${qr++}`,n=document.createElement(`div`);n.id=t,n.style.cssText=`display: flex; gap: 10px; margin-bottom: 10px; align-items: center;`,n.innerHTML=`
    ...
    `,e.appendChild(n)}async function Zr(e){e.preventDefault();let t=document.getElementById(`fmfm-extension`).value,n=document.getElementById(`fmfm-mode`).value,r=document.getElementById(`fmfm-enabled`).checked,i=document.getElementById(`fmfm-no-answer`).value,a=Array.from(document.querySelectorAll(`.fmfm-dest-number`)),o=Array.from(document.querySelectorAll(`.fmfm-dest-ringtime`)),s=a.map((e,t)=>({number:e.value,ring_time:parseInt(o[t]?.value??`20`)||20})).filter(e=>e.number);if(s.length===0){y(`At least one destination is required`,`error`);return}let c={extension:t,mode:n,enabled:r,destinations:s};i&&(c.no_answer_destination=i);try{let e=await(await g(`${m()}/api/fmfm/config`,{method:`POST`,headers:h(),body:JSON.stringify(c)})).json();e.success?(y(`FMFM configured for extension ${t}`,`success`),Xr(),Jr()):y(e.error||`Error configuring FMFM`,`error`)}catch(e){console.error(`Error saving FMFM config:`,e),y(`Error saving FMFM configuration`,`error`)}}function Qr(e){Yr();let t=document.getElementById(`fmfm-extension`);t&&(t.value=e.extension,t.readOnly=!0);let n=document.getElementById(`fmfm-mode`);n&&(n.value=e.mode);let r=document.getElementById(`fmfm-enabled`);r&&(r.checked=e.enabled!==!1);let i=document.getElementById(`fmfm-no-answer`);i&&(i.value=e.no_answer_destination||``);let a=document.getElementById(`fmfm-destinations-list`);if(a)if(a.innerHTML=``,e.destinations&&e.destinations.length>0)for(let t of e.destinations){L();let e=a.children,n=e[e.length-1],r=n.querySelector(`.fmfm-dest-number`);r&&(r.value=t.number);let i=n.querySelector(`.fmfm-dest-ringtime`);i&&(i.value=String(t.ring_time??20))}else L()}async function $r(e){if(confirm(`Are you sure you want to delete FMFM configuration for extension ${e}?`))try{let t=await(await g(`${m()}/api/fmfm/config/${e}`,{method:`DELETE`,headers:h()})).json();t.success?(y(`FMFM configuration deleted for ${e}`,`success`),Jr()):y(t.error||`Error deleting FMFM configuration`,`error`)}catch(e){console.error(`Error deleting FMFM config:`,e),y(`Error deleting FMFM configuration`,`error`)}}function ei(e){let t=[];if(e.days_of_week){let n=[`Mon`,`Tue`,`Wed`,`Thu`,`Fri`,`Sat`,`Sun`],r=e.days_of_week.map(e=>n[e]).join(`, `);t.push(r)}return e.start_time&&e.end_time&&t.push(`${e.start_time}-${e.end_time}`),e.holidays===!0?t.push(`Holidays`):e.holidays===!1&&t.push(`Non-holidays`),t.length>0?t.join(` | `):`Always`}async function ti(){try{let e=await g(`${m()}/api/time-routing/rules`,{headers:h()});if(!e.ok)throw Error(`HTTP ${e.status}: ${e.statusText}`);let t=await e.json();if(t.rules){let e=document.getElementById(`time-routing-total`);e&&(e.textContent=String(t.count||0));let n=t.rules.filter(e=>e.enabled!==!1).length,r=t.rules.filter(e=>e.name&&(e.name.toLowerCase().includes(`business`)||e.name.toLowerCase().includes(`hours`))).length,i=t.rules.filter(e=>e.name&&(e.name.toLowerCase().includes(`after`)||e.name.toLowerCase().includes(`closed`))).length,a=document.getElementById(`time-routing-active`);a&&(a.textContent=String(n));let o=document.getElementById(`time-routing-business`);o&&(o.textContent=String(r));let s=document.getElementById(`time-routing-after`);s&&(s.textContent=String(i));let c=document.getElementById(`time-routing-list`);if(!c)return;t.rules.length===0?c.innerHTML=`<tr><td colspan="7" style="text-align: center;">No time-based routing rules</td></tr>`:c.innerHTML=t.rules.map(e=>{let t=e.enabled===!1?`<span class="badge" style="background: #6b7280;">Disabled</span>`:`<span class="badge" style="background: #10b981;">Active</span>`,n=ei(e.time_conditions||{});return`
                    ...
                    `}).join(``)}}catch(e){console.error(`Error loading time routing rules:`,e),y(`Error loading time routing rules`,`error`)}}function ni(){let e=document.getElementById(`add-time-rule-modal`);e&&(e.style.display=`block`)}function ri(){let e=document.getElementById(`add-time-rule-modal`);e&&(e.style.display=`none`);let t=document.getElementById(`add-time-rule-form`);t&&t.reset()}async function ii(e){e.preventDefault();let t=document.getElementById(`time-rule-name`).value,n=document.getElementById(`time-rule-destination`).value,r=document.getElementById(`time-rule-route-to`).value,i=document.getElementById(`time-rule-start`).value,a=document.getElementById(`time-rule-end`).value,o=parseInt(document.getElementById(`time-rule-priority`).value),s=document.getElementById(`time-rule-enabled`).checked,c=Array.from(document.querySelectorAll(`input[name="time-rule-days"]:checked`)).map(e=>parseInt(e.value));if(c.length===0){y(`Please select at least one day of the week`,`error`);return}let l={name:t,destination:n,route_to:r,priority:o,enabled:s,time_conditions:{days_of_week:c,start_time:i,end_time:a}};try{let e=await(await g(`${m()}/api/time-routing/rule`,{method:`POST`,headers:h(),body:JSON.stringify(l)})).json();e.success?(y(`Time routing rule "${t}" added successfully`,`success`),ri(),ti()):y(e.error||`Error adding time routing rule`,`error`)}catch(e){console.error(`Error saving time routing rule:`,e),y(`Error saving time routing rule`,`error`)}}async function ai(e,t){if(confirm(`Are you sure you want to delete time routing rule "${t}"?`))try{let n=await(await g(`${m()}/api/time-routing/rule/${e}`,{method:`DELETE`,headers:h()})).json();n.success?(y(`Time routing rule "${t}" deleted`,`success`),ti()):y(n.error||`Error deleting time routing rule`,`error`)}catch(e){console.error(`Error deleting time routing rule:`,e),y(`Error deleting time routing rule`,`error`)}}async function oi(){try{let e=await g(`${m()}/api/webhooks`,{headers:h()});if(!e.ok)throw Error(`HTTP ${e.status}: ${e.statusText}`);let t=await e.json();if(t.subscriptions){let e=document.getElementById(`webhooks-list`);if(!e)return;t.subscriptions.length===0?e.innerHTML=`<tr><td colspan="5" style="text-align: center;">No webhooks configured</td></tr>`:e.innerHTML=t.subscriptions.map(e=>{let t=e.enabled===!1?`<span class="badge" style="background: #6b7280;">Disabled</span>`:`<span class="badge" style="background: #10b981;">Active</span>`,n=(e.event_types||[]).join(`, `),r=e.secret?`Yes`:`No`;return`
                        <tr>
                            <td>
                    ...
                    `}).join(``)}}catch(e){console.error(`Error loading webhooks:`,e),y(`Error loading webhooks`,`error`)}}function si(){let e=document.getElementById(`add-webhook-modal`);e&&(e.style.display=`block`)}function ci(){let e=document.getElementById(`add-webhook-modal`);e&&(e.style.display=`none`);let t=document.getElementById(`add-webhook-form`);t&&t.reset()}async function li(e){e.preventDefault();let t=document.getElementById(`webhook-url`).value,n=document.getElementById(`webhook-secret`).value,r=document.getElementById(`webhook-enabled`).checked,i=Array.from(document.querySelectorAll(`input[name="webhook-events"]:checked`)).map(e=>e.value);if(i.length===0){y(`Please select at least one event type`,`error`);return}let a={url:t,event_types:i,enabled:r};n&&(a.secret=n);try{let e=await(await g(`${m()}/api/webhooks`,{method:`POST`,headers:h(),body:JSON.stringify(a)})).json();e.success?(y(`Webhook added successfully`,`success`),ci(),oi()):y(e.error||`Error adding webhook`,`error`)}catch(e){console.error(`Error adding webhook:`,e),y(`Error adding webhook`,`error`)}}async function ui(e){if(!confirm(`Are you sure you want to delete webhook for ${e}?`))return;let t=encodeURIComponent(e);try{let e=await(await g(`${m()}/api/webhooks/${t}`,{method:`DELETE`,headers:h()})).json();e.success?(y(`Webhook deleted`,`success`),oi()):y(e.error||`Error deleting webhook`,`error`)}catch(e){console.error(`Error deleting webhook:`,e),y(`Error deleting webhook`,`error`)}}function di(e){let t=new Date().getTime()-e.getTime(),n=Math.floor(t/(1e3*60*60)),r=Math.floor(t%(1e3*60*60)/(1e3*60));return n>0?`${n}h ${r}m`:`${r}m`}async function fi(){try{let e=await g(`${m()}/api/hot-desk/sessions`,{headers:h()});if(!e.ok)throw Error(`HTTP ${e.status}: ${e.statusText}`);let t=await e.json();if(t.sessions){let e=t.sessions.filter(e=>e.active!==!1),n=document.getElementById(`hotdesk-active`);n&&(n.textContent=String(e.length));let r=document.getElementById(`hotdesk-total`);r&&(r.textContent=String(t.sessions.length));let i=document.getElementById(`hotdesk-sessions-list`);if(!i)return;e.length===0?i.innerHTML=`<tr><td colspan="6" style="text-align: center;">No active hot desk sessions</td></tr>`:i.innerHTML=e.map(e=>{let t=e.login_time?new Date(e.login_time).toLocaleString():`N/A`,n=e.login_time?di(new Date(e.login_time)):`N/A`;return`
                        <tr>
                            <td><strong>${_(e.extension)}</strong></td>
                    ...
                    `}).join(``)}}catch(e){console.error(`Error loading callback queue:`,e),y(`Error loading callback queue`,`error`)}}function yi(){let e=document.createElement(`div`);e.className=`modal`,e.id=`request-callback-modal`,e.innerHTML=`
        <div class="modal-content">
    ...
    `,document.body.appendChild(e),e.style.display=`block`}function bi(){let e=document.getElementById(`request-callback-modal`);e&&e.remove()}async function xi(e){e.preventDefault();let t=document.getElementById(`callback-queue-id`).value,n=document.getElementById(`callback-caller-number`).value,r=document.getElementById(`callback-caller-name`).value,i=document.getElementById(`callback-preferred-time`).value,a={queue_id:t,caller_number:n};r&&(a.caller_name=r),i&&(a.preferred_time=new Date(i).toISOString());try{let e=await(await g(`${m()}/api/callback-queue/request`,{method:`POST`,headers:h(),body:JSON.stringify(a)})).json();e.success?(y(`Callback requested successfully`,`success`),bi(),R()):y(e.error||`Error requesting callback`,`error`)}catch(e){console.error(`Error requesting callback:`,e),y(`Error requesting callback`,`error`)}}async function Si(e){let t=prompt(`Enter your agent ID/extension:`);if(t)try{let n=await(await g(`${m()}/api/callback-queue/start`,{method:`POST`,headers:h(),body:JSON.stringify({callback_id:e,agent_id:t})})).json();n.success?(y(`Started callback to ${n.caller_number??`caller`}`,`success`),R()):y(n.error||`Error starting callback`,`error`)}catch(e){console.error(`Error starting callback:`,e),y(`Error starting callback`,`error`)}}async function Ci(e,t){let n=``;t||(n=prompt(`Enter reason for failure (optional):`)||``);try{let r=await(await g(`${m()}/api/callback-queue/complete`,{method:`POST`,headers:h(),body:JSON.stringify({callback_id:e,success:t,notes:n})})).json();r.success?(y(t?`Callback completed`:`Callback will be retried`,`success`),R()):y(r.error||`Error completing callback`,`error`)}catch(e){console.error(`Error completing callback:`,e),y(`Error completing callback`,`error`)}}async function wi(e){if(confirm(`Are you sure you want to cancel this callback request?`))try{let t=await(await g(`${m()}/api/callback-queue/cancel`,{method:`POST`,headers:h(),body:JSON.stringify({callback_id:e})})).json();t.success?(y(`Callback cancelled`,`success`),R()):y(t.error||`Error cancelling callback`,`error`)}catch(e){console.error(`Error cancelling callback:`,e),y(`Error cancelling callback`,`error`)}}window.loadFMFMExtensions=Jr,window.showAddFMFMModal=Yr,window.closeAddFMFMModal=Xr,window.addFMFMDestinationRow=L,window.saveFMFMConfig=Zr,window.editFMFMConfig=Qr,window.deleteFMFMConfig=$r,window.getScheduleDescription=ei,window.showAddTimeRuleModal=ni,window.closeAddTimeRuleModal=ri,window.loadTimeRoutingRules=ti,window.saveTimeRoutingRule=ii,window.deleteTimeRoutingRule=ai,window.showAddWebhookModal=si,window.closeAddWebhookModal=ci,window.loadWebhooks=oi,window.addWebhook=li,window.deleteWebhook=ui,window.loadHotDeskSessions=fi,window.logoutHotDesk=pi,window.getDuration=di,window.loadRetentionPolicies=mi,window.showAddRetentionPolicyModal=hi,window.closeAddRetentionPolicyModal=gi,window.addRetentionPolicy=_i,window.deleteRetentionPolicy=vi,window.loadCallbackQueue=R,window.showRequestCallbackModal=yi,window.closeRequestCallbackModal=bi,window.requestCallback=xi,window.startCallback=Si,window.completeCallback=Ci,window.cancelCallback=wi;async function z(){try{let e=m(),[t,n]=await Promise.all([g(`${e}/api/fraud-detection/alerts?hours=24`,{headers:h()}),g(`${e}/api/fraud-detection/statistics`,{headers:h()})]),[r,i]=await Promise.all([t.json(),n.json()]);if(i){let e=e=>document.getElementById(e);e(`fraud-total-alerts`)&&(e(`fraud-total-alerts`).textContent=String(i.total_alerts??0)),e(`fraud-high-risk`)&&(e(`fraud-high-risk`).textContent=String(i.high_risk_alerts??0)),e(`fraud-blocked-patterns`)&&(e(`fraud-blocked-patterns`).textContent=String(i.blocked_patterns_count??0)),e(`fraud-extensions-flagged`)&&(e(`fraud-extensions-flagged`).textContent=String(i.extensions_flagged??0))}if(r?.alerts){let e=document.getElementById(`fraud-alerts-list`);e&&(r.alerts.length===0?e.innerHTML=`<tr><td colspan="5" style="text-align: center;">No fraud alerts detected</td></tr>`:e.innerHTML=r.alerts.map(e=>{let t=new Date(e.timestamp).toLocaleString(),n=e.fraud_score>.8?`#ef4444`:e.fraud_score>.5?`#f59e0b`:`#10b981`,r=(e.fraud_score*100).toFixed(0),i=(e.alert_types??[]).join(`, `);return`
                            <tr>
                    ...
                    `).join(``))}}catch(e){console.error(`Error loading fraud detection data:`,e),y(`Error loading fraud detection data`,`error`)}}function Ti(){let e=document.getElementById(`add-blocked-pattern-modal`);e&&(e.style.display=`block`)}function Ei(){let e=document.getElementById(`add-blocked-pattern-modal`);e&&(e.style.display=`none`);let t=document.getElementById(`add-blocked-pattern-form`);t&&t.reset()}async function Di(e){e.preventDefault();let t=document.getElementById(`blocked-pattern`),n=document.getElementById(`blocked-reason`),r=t?.value??``,i=n?.value??``;try{new RegExp(r)}catch(e){y(`Invalid regex pattern: ${e instanceof Error?e.message:String(e)}`,`error`);return}let a={pattern:r,reason:i};try{let e=await(await g(`${m()}/api/fraud-detection/blocked-pattern`,{method:`POST`,headers:h(),body:JSON.stringify(a)})).json();e.success?(y(`Blocked pattern added successfully`,`success`),Ei(),z()):y(e.error??`Error adding blocked pattern`,`error`)}catch(e){console.error(`Error adding blocked pattern:`,e),y(`Error adding blocked pattern`,`error`)}}async function Oi(e,t){if(confirm(`Are you sure you want to unblock pattern "${t}"?`))try{let t=await(await g(`${m()}/api/fraud-detection/blocked-pattern/${e}`,{method:`DELETE`,headers:h()})).json();t.success?(y(`Blocked pattern removed`,`success`),z()):y(t.error??`Error removing blocked pattern`,`error`)}catch(e){console.error(`Error removing blocked pattern:`,e),y(`Error removing blocked pattern`,`error`)}}async function B(){try{let e=m(),[t,n]=await Promise.all([g(`${e}/api/callback-queue/list`,{headers:h()}),g(`${e}/api/callback-queue/statistics`,{headers:h()})]),[r,i]=await Promise.all([t.json(),n.json()]);if(i){let e=e=>document.getElementById(e);e(`callback-total`)&&(e(`callback-total`).textContent=String(i.total_callbacks??0));let t=i.status_breakdown??{};e(`callback-scheduled`)&&(e(`callback-scheduled`).textContent=String(t.scheduled??0)),e(`callback-in-progress`)&&(e(`callback-in-progress`).textContent=String(t.in_progress??0)),e(`callback-completed`)&&(e(`callback-completed`).textContent=String(t.completed??0)),e(`callback-failed`)&&(e(`callback-failed`).textContent=String(t.failed??0))}if(r?.callbacks){let e=document.getElementById(`callback-list`);e&&(r.callbacks.length===0?e.innerHTML=`<tr><td colspan="8" style="text-align: center;">No callbacks in queue</td></tr>`:e.innerHTML=r.callbacks.map(e=>{let t=new Date(e.requested_at).toLocaleString(),n=new Date(e.callback_time).toLocaleString(),r=``;switch(e.status){case`scheduled`:r=`badge-info`;break;case`in_progress`:r=`badge-warning`;break;case`completed`:r=`badge-success`;break;case`failed`:r=`badge-danger`;break;case`cancelled`:r=`badge-secondary`;break;default:r=`badge-info`}return`
                            <tr>
                                <td><code>${_(e.callback_id)}</code></td>
                                <td>${_(e.queue_id)}</td>
                                <td>
                        ...
                        `}).join(``))}}catch(e){console.error(`Error loading callback queue:`,e),y(`Error loading callback queue`,`error`)}}function ki(){let e=document.createElement(`div`);e.className=`modal`,e.id=`request-callback-modal`,e.innerHTML=`
        <div class="modal-content">
            <span class="close" onclick="closeRequestCallbackModal()">&times;</span>
            <h2>Request Callback</h2>
            <form id="request-callback-form" onsubmit="requestCallback(event)">
    ...
    `,document.body.appendChild(e),e.style.display=`block`}function Ai(){let e=document.getElementById(`request-callback-modal`);e&&e.remove()}async function ji(e){e.preventDefault();let t=document.getElementById(`callback-queue-id`)?.value??``,n=document.getElementById(`callback-caller-number`)?.value??``,r=document.getElementById(`callback-caller-name`)?.value??``,i=document.getElementById(`callback-preferred-time`)?.value??``,a={queue_id:t,caller_number:n};r&&(a.caller_name=r),i&&(a.preferred_time=new Date(i).toISOString());try{let e=await(await g(`${m()}/api/callback-queue/request`,{method:`POST`,headers:h(),body:JSON.stringify(a)})).json();e.success?(y(`Callback requested successfully`,`success`),Ai(),B()):y(e.error??`Error requesting callback`,`error`)}catch(e){console.error(`Error requesting callback:`,e),y(`Error requesting callback`,`error`)}}async function Mi(e){let t=prompt(`Enter your agent ID/extension:`);if(t)try{let n=await(await g(`${m()}/api/callback-queue/start`,{method:`POST`,headers:h(),body:JSON.stringify({callback_id:e,agent_id:t})})).json();n.success?(y(`Started callback to ${n.caller_number??e}`,`success`),B()):y(n.error??`Error starting callback`,`error`)}catch(e){console.error(`Error starting callback:`,e),y(`Error starting callback`,`error`)}}async function Ni(e,t){let n=``;t||(n=prompt(`Enter reason for failure (optional):`)??``);try{let r=await(await g(`${m()}/api/callback-queue/complete`,{method:`POST`,headers:h(),body:JSON.stringify({callback_id:e,success:t,notes:n})})).json();r.success?(y(t?`Callback completed`:`Callback will be retried`,`success`),B()):y(r.error??`Error completing callback`,`error`)}catch(e){console.error(`Error completing callback:`,e),y(`Error completing callback`,`error`)}}async function Pi(e){if(confirm(`Are you sure you want to cancel this callback request?`))try{let t=await(await g(`${m()}/api/callback-queue/cancel`,{method:`POST`,headers:h(),body:JSON.stringify({callback_id:e})})).json();t.success?(y(`Callback cancelled`,`success`),B()):y(t.error??`Error cancelling callback`,`error`)}catch(e){console.error(`Error cancelling callback:`,e),y(`Error cancelling callback`,`error`)}}async function Fi(){try{let e=m(),[t,n,r]=await Promise.all([g(`${e}/api/mobile-push/devices`,{headers:h()}),g(`${e}/api/mobile-push/statistics`,{headers:h()}),g(`${e}/api/mobile-push/history`,{headers:h()})]),[i,a,o]=await Promise.all([t.json(),n.json(),r.json()]);if(a){let e=e=>document.getElementById(e);e(`push-total-devices`)&&(e(`push-total-devices`).textContent=String(a.total_devices??0)),e(`push-total-users`)&&(e(`push-total-users`).textContent=String(a.total_users??0));let t=a.platforms??{};e(`push-ios-devices`)&&(e(`push-ios-devices`).textContent=String(t.ios??0)),e(`push-android-devices`)&&(e(`push-android-devices`).textContent=String(t.android??0)),e(`push-recent-notifications`)&&(e(`push-recent-notifications`).textContent=String(a.recent_notifications??0))}if(i?.devices){let e=document.getElementById(`mobile-devices-list`);e&&(i.devices.length===0?e.innerHTML=`<tr><td colspan="5" style="text-align: center;">No devices registered</td></tr>`:e.innerHTML=i.devices.map(e=>{let t=new Date(e.registered_at).toLocaleString(),n=new Date(e.last_seen).toLocaleString(),r=``;return r=e.platform===`ios`?`<span class="badge badge-info">iOS</span>`:e.platform===`android`?`<span class="badge badge-success">Android</span>`:`<span class="badge badge-secondary">${_(e.platform)}</span>`,`
                            <tr>
                        ...
                        `}).join(``))}if(o?.history){let e=document.getElementById(`push-history-list`);e&&(o.history.length===0?e.innerHTML=`<tr><td colspan="5" style="text-align: center;">No notifications sent</td></tr>`:e.innerHTML=o.history.slice(0,50).map(e=>{let t=new Date(e.sent_at).toLocaleString(),n=e.success_count??0,r=e.failure_count??0;return`
                            ...
                            `}).join(``))}}catch(e){console.error(`Error loading mobile push data:`,e),y(`Error loading mobile push data`,`error`)}}function Ii(){let e=document.createElement(`div`);e.className=`modal`,e.id=`register-device-modal`,e.innerHTML=`
        <div class="modal-content">
    ...
    `,document.body.appendChild(e),e.style.display=`block`}function Li(){let e=document.getElementById(`register-device-modal`);e&&e.remove()}async function Ri(e){e.preventDefault();let t={user_id:document.getElementById(`device-user-id`)?.value??``,device_token:document.getElementById(`device-token`)?.value.trim()??``,platform:document.getElementById(`device-platform`)?.value??``};try{let e=await(await g(`${m()}/api/mobile-push/register`,{method:`POST`,headers:h(),body:JSON.stringify(t)})).json();e.success?(y(`Device registered successfully`,`success`),Li(),Fi()):y(e.error??`Error registering device`,`error`)}catch(e){console.error(`Error registering device:`,e),y(`Error registering device`,`error`)}}function zi(){let e=document.createElement(`div`);e.className=`modal`,e.id=`test-notification-modal`,e.innerHTML=`
        <div class="modal-content">
    ...
    `,document.body.appendChild(e),e.style.display=`block`}function Bi(){let e=document.getElementById(`test-notification-modal`);e&&e.remove()}function Vi(e){e.preventDefault(),Hi(document.getElementById(`test-user-id`)?.value??``),Bi()}async function Hi(e){try{let t=await(await g(`${m()}/api/mobile-push/test`,{method:`POST`,headers:h(),body:JSON.stringify({user_id:e})})).json();t.success||t.stub_mode?(t.stub_mode?y(`Test notification logged (Firebase not configured)`,`warning`):y(`Test notification sent: ${t.success_count??0} succeeded, ${t.failure_count??0} failed`,`success`),Fi()):y(t.error??`Error sending test notification`,`error`)}catch(e){console.error(`Error sending test notification:`,e),y(`Error sending test notification`,`error`)}}async function Ui(){try{let e=m(),[t,n]=await Promise.all([g(`${e}/api/recording-announcements/statistics`,{headers:h()}),g(`${e}/api/recording-announcements/config`,{headers:h()})]),[r,i]=await Promise.all([t.json(),n.json()]);if(r){let e=e=>document.getElementById(e);e(`announcements-enabled`)&&(e(`announcements-enabled`).textContent=r.enabled?`Enabled`:`Disabled`),e(`announcements-played`)&&(e(`announcements-played`).textContent=String(r.announcements_played??0)),e(`consent-accepted`)&&(e(`consent-accepted`).textContent=String(r.consent_accepted??0)),e(`consent-declined`)&&(e(`consent-declined`).textContent=String(r.consent_declined??0)),e(`announcement-type`)&&(e(`announcement-type`).textContent=r.announcement_type??`N/A`),e(`require-consent`)&&(e(`require-consent`).textContent=r.require_consent?`Yes`:`No`)}if(i){let e=e=>document.getElementById(e);e(`audio-file-path`)&&(e(`audio-file-path`).textContent=i.audio_path??`N/A`),e(`announcement-text`)&&(e(`announcement-text`).textContent=i.announcement_text??`N/A`)}}catch(e){console.error(`Error loading recording announcements data:`,e),y(`Error loading recording announcements data`,`error`)}}async function Wi(){try{let e=await(await g(`${m()}/api/framework/speech-analytics/configs`,{headers:h()})).json(),t=document.getElementById(`speech-analytics-configs-table`);if(!t)return;if(!e.configs||e.configs.length===0){t.innerHTML=`<tr><td colspan="5" class="loading">No extension-specific configurations. Using system defaults.</td></tr>`;return}t.innerHTML=e.configs.map(e=>`
            <tr>
                <td>${_(e.extension)}</td>
        ...
        `).join(``)}catch(e){console.error(`Error loading speech analytics configs:`,e),y(`Error loading speech analytics configurations`,`error`)}}async function Gi(){try{let e=await g(`${m()}/api/framework/integrations/activity-log`,{headers:h()});if(!e.ok){console.error(`Error loading CRM activity log:`,e.status),y(`Error loading CRM activity log`,`error`);return}let t=await e.json(),n=document.getElementById(`crm-activity-log-table`);if(!n)return;if(!t.activities||t.activities.length===0){n.innerHTML=`<tr><td colspan="5" class="loading">No integration activity yet</td></tr>`;return}n.innerHTML=t.activities.map(e=>{let t=e.status===`success`?`success`:`error`,n=e.status===`success`?`OK`:`FAIL`;return`
            ...
            `}).join(``)}catch(e){console.error(`Error loading CRM activity log:`,e),y(`Error loading CRM activity log`,`error`)}}async function Ki(){if(confirm(`Clear old activity log entries? This will remove entries older than 30 days.`))try{let e=await(await g(`${m()}/api/framework/integrations/activity-log/clear`,{method:`POST`,headers:h()})).json();e.success?(y(`Cleared ${e.deleted_count??0} old entries`,`success`),Gi()):y(e.error??`Error clearing activity log`,`error`)}catch(e){console.error(`Error clearing CRM activity log:`,e),y(`Error clearing activity log`,`error`)}}async function qi(){try{let e=await g(`${m()}/api/framework/compliance/soc2/controls`,{headers:h()}),t=e.ok?await e.json():null,n=document.getElementById(`compliance-soc2-count`);if(n&&t){let e=t.controls??[];n.textContent=String(e.length)}}catch(e){console.error(`Error loading compliance data:`,e)}}window.loadFraudAlerts=z,window.showAddBlockedPatternModal=Ti,window.closeAddBlockedPatternModal=Ei,window.addBlockedPattern=Di,window.deleteBlockedPattern=Oi,window.loadCallbackQueue=B,window.showRequestCallbackModal=ki,window.closeRequestCallbackModal=Ai,window.requestCallback=ji,window.startCallback=Mi,window.completeCallback=Ni,window.cancelCallback=Pi,window.loadMobilePushDevices=Fi,window.showRegisterDeviceModal=Ii,window.closeRegisterDeviceModal=Li,window.registerDevice=Ri,window.showTestNotificationModal=zi,window.closeTestNotificationModal=Bi,window.sendTestNotificationForm=Vi,window.sendTestNotification=Hi,window.loadRecordingAnnouncementsStats=Ui,window.loadSpeechAnalyticsConfigs=Wi,window.loadCRMActivityLog=Gi,window.clearCRMActivityLog=Ki,window.loadFraudDetectionData=z,window.loadMobilePushConfig=Fi,window.loadRecordingAnnouncements=Ui,window.loadComplianceData=qi;var Ji={completed:`success`,failed:`error`,cancelled:`warning`,busy:`warning`,"no-answer":`warning`};async function Yi(){try{let e=await g(`${m()}/api/extensions`,{headers:h()});if(!e.ok)throw Error(`HTTP ${e.status}`);return(await e.json()).extensions??[]}catch(e){return console.error(`Error loading extensions for click-to-dial:`,e),window.currentExtensions??[]}}async function Xi(){try{let e=await(await g(`${m()}/api/framework/click-to-dial/configs`,{headers:h()})).json();if(e.error){console.error(`Error loading click-to-dial configs:`,e.error);return}let t=await Yi(),n=document.getElementById(`ctd-extension-select`),r=document.getElementById(`ctd-history-extension`);if(n&&t.length>0){n.innerHTML=`<option value="">Select Extension</option>`;for(let e of t){let t=document.createElement(`option`);t.value=e.number,t.textContent=`${e.number} - ${e.name}`,n.appendChild(t)}}else n&&(n.innerHTML=`<option value="">No extensions available</option>`);if(r&&t.length>0){r.innerHTML=`<option value="">All Extensions</option>`;for(let e of t){let t=document.createElement(`option`);t.value=e.number,t.textContent=`${e.number} - ${e.name}`,r.appendChild(t)}}else r&&(r.innerHTML=`<option value="">No extensions available</option>`);let i=document.getElementById(`ctd-configs-table`);if(!i)return;if(!e.configs||e.configs.length===0){i.innerHTML=`<tr><td colspan="6" style="text-align: center;">No configurations found. Configure extensions above.</td></tr>`;return}i.innerHTML=e.configs.map(e=>`
            <tr>
                <td>${_(e.extension)}</td>
        ...
        `).join(``)}catch(e){console.error(`Error loading extension locations:`,e),y(`Error loading extension locations`,`error`)}}async function sa(){let e=document.getElementById(`location-history-extension`)?.value??``,t=e?`/api/framework/nomadic-e911/history/${e}`:`/api/framework/nomadic-e911/history`;try{let e=await(await g(`${m()}${t}`,{headers:h()})).json(),n=document.getElementById(`location-history-table`);if(!n)return;if(!e.history||e.history.length===0){n.innerHTML=`<tr><td colspan="5" class="loading">No location history available</td></tr>`;return}n.innerHTML=e.history.map(e=>`
            <tr>
                <td>${_(new Date(e.timestamp).toLocaleString())}</td>
                <td>${_(e.extension)}</td>
        ...
        `).join(``)}catch(e){console.error(`Error loading location history:`,e),y(`Error loading location history`,`error`)}}function ca(){let e=document.getElementById(`e911-site-modal`);e&&e.remove()}function la(e){return`
    ...
    `}async function ua(e){let t=new FormData(e),n=t.get(`site_id`),r={};for(let[e,n]of t.entries())e!==`site_id`&&String(n).trim()&&(r[e]=String(n).trim());try{let e=m(),t=await(await g(n?`${e}/api/framework/nomadic-e911/sites/${n}`:`${e}/api/framework/nomadic-e911/create-site`,{method:n?`PUT`:`POST`,headers:h(),body:JSON.stringify(r)})).json();t.success?(y(`E911 site saved successfully`,`success`),ca(),await aa()):y(t.error??`Error saving E911 site`,`error`)}catch(e){console.error(`Error saving E911 site:`,e),y(`Error saving E911 site`,`error`)}}function da(){ca(),document.body.insertAdjacentHTML(`beforeend`,la());let e=document.getElementById(`e911-site-form`);e.onsubmit=t=>{t.preventDefault(),ua(e)}}function fa(e){g(`${m()}/api/framework/nomadic-e911/sites`,{headers:h()}).then(async t=>{let n=(await t.json()).sites?.find(t=>t.id===e);if(!n){y(`Site not found`,`error`);return}ca(),document.body.insertAdjacentHTML(`beforeend`,la(n));let r=document.getElementById(`e911-site-form`);r.onsubmit=e=>{e.preventDefault(),ua(r)}}).catch(()=>{y(`Error loading site details`,`error`)})}async function pa(e){if(confirm(`Delete E911 site ${e}?`))try{let t=await(await g(`${m()}/api/framework/nomadic-e911/sites/${e}`,{method:`DELETE`,headers:h()})).json();t.success?(y(`E911 site deleted`,`success`),await aa()):y(t.error??`Error deleting site`,`error`)}catch(e){console.error(`Error deleting E911 site:`,e),y(`Error deleting E911 site`,`error`)}}function ma(){let e=document.getElementById(`location-update-modal`);e&&e.remove()}function ha(e){return`
    ...
    `}async function ga(e){let t=new FormData(e),n=String(t.get(`extension`)??``).trim();if(!n){y(`Extension is required`,`error`);return}let r={};for(let[e,n]of t.entries())e!==`extension`&&String(n).trim()&&(r[e]=String(n).trim());try{let e=await(await g(`${m()}/api/framework/nomadic-e911/update-location/${encodeURIComponent(n)}`,{method:`POST`,headers:h(),body:JSON.stringify(r)})).json();e.success?(y(`Location updated for extension ${n}`,`success`),ma(),await oa()):y(e.error??`Error updating location`,`error`)}catch(e){console.error(`Error updating location:`,e),y(`Error updating location`,`error`)}}function _a(){ma(),document.body.insertAdjacentHTML(`beforeend`,ha());let e=document.getElementById(`location-update-form`);e.onsubmit=t=>{t.preventDefault(),ga(e)}}function va(e){ma(),document.body.insertAdjacentHTML(`beforeend`,ha(e));let t=document.getElementById(`location-update-form`);t.onsubmit=e=>{e.preventDefault(),ga(t)}}function ya(){let e=document.getElementById(`speech-config-modal`);e&&e.remove()}function ba(e,t){return`
    ...
    `}async function xa(e){let t=new FormData(e),n=String(t.get(`extension`)??``).trim();if(!n){y(`Extension is required`,`error`);return}let r={enabled:t.get(`enabled`)===`on`,transcription_enabled:t.get(`transcription_enabled`)===`on`,sentiment_enabled:t.get(`sentiment_enabled`)===`on`,summarization_enabled:t.get(`summarization_enabled`)===`on`,keywords:String(t.get(`keywords`)??``).trim(),alert_threshold:parseFloat(String(t.get(`alert_threshold`)??`0.7`))};try{let e=await(await g(`${m()}/api/framework/speech-analytics/config/${encodeURIComponent(n)}`,{method:`POST`,headers:h(),body:JSON.stringify(r)})).json();e.success?(y(`Speech analytics config saved`,`success`),ya()):y(e.error??`Error saving config`,`error`)}catch(e){console.error(`Error saving speech analytics config:`,e),y(`Error saving speech analytics config`,`error`)}}function Sa(){ya(),document.body.insertAdjacentHTML(`beforeend`,ba());let e=document.getElementById(`speech-config-form`);e.onsubmit=t=>{t.preventDefault(),xa(e)}}function Ca(e){ya(),document.body.insertAdjacentHTML(`beforeend`,ba(e));let t=document.getElementById(`speech-config-form`);t.onsubmit=e=>{e.preventDefault(),xa(t)}}async function wa(e){if(confirm(`Delete speech analytics config for extension ${e}?`))try{let t=await(await g(`${m()}/api/framework/speech-analytics/config/${encodeURIComponent(e)}`,{method:`DELETE`,headers:h()})).json();t.success?y(`Speech analytics config deleted`,`success`):y(t.error??`Error deleting config`,`error`)}catch(e){console.error(`Error deleting speech analytics config:`,e),y(`Error deleting speech analytics config`,`error`)}}window.loadE911Sites=aa,window.loadExtensionLocations=oa,window.loadLocationHistory=sa,window.showAddE911SiteModal=da,window.editE911Site=fa,window.deleteE911Site=pa,window.removeE911SiteModal=ca,window.showUpdateLocationModal=_a,window.updateExtensionLocation=va,window.removeLocationModal=ma,window.showAddSpeechAnalyticsConfigModal=Sa,window.editSpeechAnalyticsConfig=Ca,window.deleteSpeechAnalyticsConfig=wa,window.removeSpeechConfigModal=ya,window.loadNomadicE911Data=function(){aa(),oa()};async function Ta(){await Promise.all([V(),Ea(),Da(),Oa(),ka()])}async function V(){try{let e=await g(`${m()}/api/framework/sbc/statistics`,{headers:h()},1e4);if(!e.ok)throw Error(`HTTP ${e.status}`);let t=await e.json();H(`sbc-active-sessions`,String(t.active_sessions)),H(`sbc-total-sessions`,String(t.total_sessions)),H(`sbc-blocked-requests`,String(t.blocked_requests)),H(`sbc-relayed-media`,String(t.relayed_media_mb)),H(`sbc-bandwidth-util`,`${t.bandwidth_utilization_pct}%`),H(`sbc-rate-violations`,String(t.rate_limit_violations)),H(`sbc-cac-rejections`,String(t.cac_rejections)),H(`sbc-port-pool`,`${t.relay_port_pool_size}/${t.relay_port_pool_total}`)}catch(e){console.error(`Error loading SBC statistics:`,e),y(`Failed to load SBC statistics: ${e instanceof Error?e.message:String(e)}`,`error`)}}async function Ea(){try{let e=await g(`${m()}/api/framework/sbc/config`,{headers:h()},1e4);if(!e.ok)throw Error(`HTTP ${e.status}`);let t=await e.json();La(`sbc-cfg-enabled`,String(t.enabled)),La(`sbc-cfg-topology-hiding`,String(t.topology_hiding)),La(`sbc-cfg-media-relay`,String(t.media_relay)),La(`sbc-cfg-stun`,String(t.stun_enabled)),Ra(`sbc-cfg-public-ip`,t.public_ip),Ra(`sbc-cfg-max-calls`,String(t.max_calls)),Ra(`sbc-cfg-max-bandwidth`,String(t.max_bandwidth)),Ra(`sbc-cfg-rate-limit`,String(t.rate_limit))}catch(e){console.error(`Error loading SBC config:`,e)}}async function Da(){try{let e=await g(`${m()}/api/framework/sbc/relays`,{headers:h()},1e4);if(!e.ok)throw Error(`HTTP ${e.status}`);let t=await e.json(),n=document.getElementById(`sbc-relays-list`);if(!n)return;let r=Object.values(t.relays??{});if(r.length===0){n.innerHTML=`<tr><td colspan="7" style="text-align: center;">No active relays</td></tr>`;return}n.innerHTML=r.map(e=>`
      <tr>
    ...
    `).join(``)}catch(e){console.error(`Error loading SBC relays:`,e);let t=document.getElementById(`sbc-relays-list`);t&&(t.innerHTML=`<tr><td colspan="7" style="text-align: center;">Error loading relays</td></tr>`)}}async function Oa(){try{let e=await g(`${m()}/api/framework/sbc/blacklist`,{headers:h()},1e4);if(!e.ok)throw Error(`HTTP ${e.status}`);Ba(`sbc-blacklist-table`,(await e.json()).blacklist,`removeSBCBlacklist`)}catch(e){console.error(`Error loading SBC blacklist:`,e)}}async function ka(){try{let e=await g(`${m()}/api/framework/sbc/whitelist`,{headers:h()},1e4);if(!e.ok)throw Error(`HTTP ${e.status}`);Ba(`sbc-whitelist-table`,(await e.json()).whitelist,`removeSBCWhitelist`)}catch(e){console.error(`Error loading SBC whitelist:`,e)}}async function Aa(){try{let e=m(),t={enabled:za(`sbc-cfg-enabled`)===`true`,topology_hiding:za(`sbc-cfg-topology-hiding`)===`true`,media_relay:za(`sbc-cfg-media-relay`)===`true`,stun_enabled:za(`sbc-cfg-stun`)===`true`,public_ip:U(`sbc-cfg-public-ip`),max_calls:parseInt(U(`sbc-cfg-max-calls`)||`1000`,10),max_bandwidth:parseInt(U(`sbc-cfg-max-bandwidth`)||`100000`,10),rate_limit:parseInt(U(`sbc-cfg-rate-limit`)||`100`,10)},n=await g(`${e}/api/framework/sbc/config`,{method:`PUT`,headers:{...h(),"Content-Type":`application/json`},body:JSON.stringify(t)},1e4);if(!n.ok)throw Error(`HTTP ${n.status}`);y(`Warden SBC configuration saved`,`success`),await V()}catch(e){console.error(`Error saving SBC config:`,e),y(`Failed to save SBC configuration`,`error`)}}async function ja(e){if(confirm(`Terminate relay for call ${e}?`))try{let t=await g(`${m()}/api/framework/sbc/relay/${encodeURIComponent(e)}`,{method:`DELETE`,headers:h()},1e4);if(!t.ok)throw Error(`HTTP ${t.status}`);y(`Relay terminated`,`success`),await Da(),await V()}catch(e){console.error(`Error terminating relay:`,e),y(`Failed to terminate relay`,`error`)}}async function Ma(){let e=document.getElementById(`sbc-blacklist-ip`),t=e?.value?.trim();if(!t){y(`Enter an IP address`,`warning`);return}try{let n=await g(`${m()}/api/framework/sbc/blacklist`,{method:`POST`,headers:{...h(),"Content-Type":`application/json`},body:JSON.stringify({ip:t})},1e4);if(!n.ok)throw Error(`HTTP ${n.status}`);e&&(e.value=``),y(`${t} added to blacklist`,`success`),await Oa(),await V()}catch(e){console.error(`Error adding to blacklist:`,e),y(`Failed to add to blacklist`,`error`)}}async function Na(e){try{let t=await g(`${m()}/api/framework/sbc/blacklist/${encodeURIComponent(e)}`,{method:`DELETE`,headers:h()},1e4);if(!t.ok)throw Error(`HTTP ${t.status}`);y(`${e} removed from blacklist`,`success`),await Oa(),await V()}catch(e){console.error(`Error removing from blacklist:`,e),y(`Failed to remove from blacklist`,`error`)}}async function Pa(){let e=document.getElementById(`sbc-whitelist-ip`),t=e?.value?.trim();if(!t){y(`Enter an IP address`,`warning`);return}try{let n=await g(`${m()}/api/framework/sbc/whitelist`,{method:`POST`,headers:{...h(),"Content-Type":`application/json`},body:JSON.stringify({ip:t})},1e4);if(!n.ok)throw Error(`HTTP ${n.status}`);e&&(e.value=``),y(`${t} added to whitelist`,`success`),await ka(),await V()}catch(e){console.error(`Error adding to whitelist:`,e),y(`Failed to add to whitelist`,`error`)}}async function Fa(e){try{let t=await g(`${m()}/api/framework/sbc/whitelist/${encodeURIComponent(e)}`,{method:`DELETE`,headers:h()},1e4);if(!t.ok)throw Error(`HTTP ${t.status}`);y(`${e} removed from whitelist`,`success`),await ka(),await V()}catch(e){console.error(`Error removing from whitelist:`,e),y(`Failed to remove from whitelist`,`error`)}}async function Ia(){let e=U(`sbc-nat-local-ip`),t=U(`sbc-nat-public-ip`);if(!e||!t){y(`Enter both local and public IP addresses`,`warning`);return}try{let n=await g(`${m()}/api/framework/sbc/nat-detect`,{method:`POST`,headers:{...h(),"Content-Type":`application/json`},body:JSON.stringify({local_ip:e,public_ip:t})},15e3);if(!n.ok)throw Error(`HTTP ${n.status}`);let r=await n.json(),i=document.getElementById(`sbc-nat-result`),a=document.getElementById(`sbc-nat-type`);i&&(i.style.display=`block`),a&&(a.textContent=r.nat_type),y(`NAT type detected: ${r.nat_type}`,`success`)}catch(e){console.error(`Error detecting NAT:`,e),y(`NAT detection failed`,`error`)}}function H(e,t){let n=document.getElementById(e);n&&(n.textContent=t)}function La(e,t){let n=document.getElementById(e);n&&(n.value=t)}function Ra(e,t){let n=document.getElementById(e);n&&(n.value=t)}function za(e){return document.getElementById(e)?.value??``}function U(e){return document.getElementById(e)?.value?.trim()??``}function Ba(e,t,n){let r=document.getElementById(e);if(r){if(t.length===0){r.innerHTML=`<tr><td colspan="2" style="text-align: center;">No entries</td></tr>`;return}r.innerHTML=t.map(e=>`
    <tr>
      <td><code>${_(e)}</code></td>
      <td>
  ...
  `).join(``)}}window.loadSBCData=Ta,window.saveSBCConfig=Aa,window.terminateSBCRelay=ja,window.addSBCBlacklist=Ma,window.removeSBCBlacklist=Na,window.addSBCWhitelist=Pa,window.removeSBCWhitelist=Fa,window.detectSBCNat=Ia,window.fetchWithTimeout=g,window.getAuthHeaders=h,window.getApiBaseUrl=m,window.DEFAULT_FETCH_TIMEOUT=Ue,window.store=We,window.showNotification=y,window.displayError=b,window.setSuppressErrorNotifications=rt,window.showTab=S,window.switchTab=S,window.initializeTabs=lt,window.escapeHtml=_,window.copyToClipboard=Ge,window.formatDate=Ke,window.truncate=qe,window.getDuration=Je,window.getStatusBadge=Ye,window.getHealthBadge=Xe,window.getPriorityBadge=Ze,window.getQualityClass=Qe,window.getScheduleDescription=$e,window.downloadLicense=et,window.executeBatched=ut,window.refreshAllData=dt;var Va=`/admin/login.html`;async function Ha(){if(debugLog(`Initializing user context...`),!localStorage.getItem(`pbx_token`)){debugLog(`No authentication token found, redirecting to login...`),window.location.replace(Va);return}try{let e=await g(`${m()}/api/extensions`,{headers:h()},5e3);if(e.status===401||e.status===403){debugLog(`Authentication token is invalid, redirecting to login...`),localStorage.removeItem(`pbx_token`),localStorage.removeItem(`pbx_extension`),localStorage.removeItem(`pbx_is_admin`),localStorage.removeItem(`pbx_name`),window.location.replace(Va);return}if(!e.ok)throw Error(`HTTP ${e.status}`)}catch(e){console.error(`Error verifying authentication:`,e),y(`Unable to verify authentication - server may be starting up`,`error`)}let e=localStorage.getItem(`pbx_extension`),t=localStorage.getItem(`pbx_is_admin`)===`true`,n=localStorage.getItem(`pbx_name`)||`User`;if(!e){debugLog(`No extension number found, redirecting to login...`),window.location.replace(Va);return}We.set(`currentUser`,{number:e,is_admin:t,name:n}),debugLog(`User context initialized:`,{number:e,is_admin:t,name:n}),t?(debugLog(`Admin user - showing dashboard tab`),S(`dashboard`)):(debugLog(`Regular user - showing webrtc-phone tab`),S(`webrtc-phone`)),debugLog(`User context initialization complete`)}function Ua(){let e=document.querySelectorAll(`form[data-ajax]`);for(let t of e)t.addEventListener(`submit`,e=>{e.preventDefault(),debugLog(`Ajax form submitted:`,t.id)})}function Wa(){let e=document.getElementById(`logout-button`);e&&e.addEventListener(`click`,async()=>{let e=localStorage.getItem(`pbx_token`),t=h();localStorage.removeItem(`pbx_token`),localStorage.removeItem(`pbx_extension`),localStorage.removeItem(`pbx_is_admin`),localStorage.removeItem(`pbx_name`),localStorage.removeItem(`pbx_current_extension`);try{e&&await fetch(`${m()}/api/auth/logout`,{method:`POST`,headers:t})}catch(e){console.error(`Logout API error:`,e)}window.location.href=Va})}async function Ga(){let e=document.getElementById(`connection-status`);if(!e){console.error(`Connection status badge element not found`);return}try{if((await g(`${m()}/api/status`,{headers:h()},5e3)).ok)e.textContent=`Connected`,e.classList.remove(`connecting`,`disconnected`),e.classList.add(`connected`);else throw Error(`Connection failed`)}catch(t){console.error(`Connection check failed:`,t),e.textContent=`Disconnected`,e.classList.remove(`connecting`,`connected`),e.classList.add(`disconnected`)}}document.addEventListener(`DOMContentLoaded`,async()=>{debugLog(`DOMContentLoaded event fired - starting initialization`),Ga(),setInterval(Ga,1e4),debugLog(`Initializing tabs, forms, and logout`),lt(),Ua(),Wa(),ft();try{await Ha(),debugLog(`User context initialization complete`)}catch(e){console.error(`User context initialization failed:`,e)}debugLog(`Page initialization complete`)});async function Ka(){try{let e=await fetch(`${API_BASE}/api/auto-attendant/config`,{headers:pbxAuthHeaders()});if(e.ok){let t=await e.json();document.getElementById(`aa-enabled`).checked=t.enabled??!1,document.getElementById(`aa-extension`).value=t.extension??`0`,document.getElementById(`aa-timeout`).value=t.timeout??10,document.getElementById(`aa-max-retries`).value=t.max_retries??3}await ao(),await W(),await qa()}catch(e){console.error(`Error loading auto attendant config:`,e),showNotification(`Failed to load auto attendant configuration`,`error`)}}async function qa(){try{let e=await fetch(`${API_BASE}/api/auto-attendant/prompts`,{headers:pbxAuthHeaders()});if(!e.ok){debugWarn(`Failed to load prompts, using defaults`);return}let t=await e.json(),n=t.prompts??{},r=t.company_name??``,i=document.getElementById(`aa-company-name`);i&&(i.value=r),n.welcome&&(document.getElementById(`aa-prompt-welcome`).value=n.welcome),n.main_menu&&(document.getElementById(`aa-prompt-main-menu`).value=n.main_menu),n.invalid&&(document.getElementById(`aa-prompt-invalid`).value=n.invalid),n.timeout&&(document.getElementById(`aa-prompt-timeout`).value=n.timeout),n.transferring&&(document.getElementById(`aa-prompt-transferring`).value=n.transferring)}catch(e){console.error(`Error loading prompts:`,e)}}async function W(){try{let e=await fetch(`${API_BASE}/api/auto-attendant/menus/${K}/items`,{headers:pbxAuthHeaders()});if(e.status===404)return debugWarn(`Menu items endpoint returned 404 for menu '${K}', trying legacy API...`),await Ja();if(!e.ok){let t=await io(e);throw Error(`Failed to load menu options: ${e.status} - ${t.error||e.statusText}`)}let t=await e.json(),n=document.getElementById(`aa-menu-options-table-body`);if(Za(),!t.items||t.items.length===0){n.innerHTML=`<tr><td colspan="5" class="no-data">No menu options configured. Click "Add Menu Option" to get started.</td></tr>`;return}let r=t.items.sort((e,t)=>e.digit===t.digit?0:e.digit===`*`?1:t.digit===`*`?-1:e.digit===`#`?1:t.digit===`#`?-1:e.digit.localeCompare(t.digit));n.innerHTML=``;for(let e of r){let t=document.createElement(`tr`),r=document.createElement(`td`);r.innerHTML=`<strong>${G(e.digit)}</strong>`,t.appendChild(r);let i=document.createElement(`td`);i.innerHTML=`${Ya(e.destination_type)} ${e.destination_type}`,t.appendChild(i);let a=document.createElement(`td`);e.destination_type===`submenu`?a.innerHTML=`<span style="color: #4CAF50; font-weight: bold;">${G(e.destination_value)}</span>`:a.textContent=e.destination_value,t.appendChild(a);let o=document.createElement(`td`);o.textContent=e.description,t.appendChild(o);let s=document.createElement(`td`);if(e.destination_type===`submenu`){let t=document.createElement(`button`);t.className=`btn btn-secondary`,t.textContent=`➡️ Open`,t.onclick=()=>Xa(e.destination_value),s.appendChild(t)}let c=document.createElement(`button`);c.className=`btn btn-primary`,c.textContent=`✏️ Edit`,c.onclick=()=>eo(e.digit,e.destination_type,e.destination_value,e.description),s.appendChild(c);let l=document.createElement(`button`);l.className=`btn btn-danger`,l.textContent=`🗑️ Delete`,l.onclick=()=>no(e.digit),s.appendChild(l),t.appendChild(s),n.appendChild(t)}}catch(e){console.error(`Error loading menu options:`,e),showNotification(`Failed to load menu options`,`error`)}}async function Ja(){try{let e=await fetch(`${API_BASE}/api/auto-attendant/menu-options`,{headers:pbxAuthHeaders()});if(!e.ok)throw Error(`Failed to load menu options`);let t=await e.json(),n=document.getElementById(`aa-menu-options-table-body`);if(!t.menu_options||t.menu_options.length===0){n.innerHTML=`<tr><td colspan="5" class="no-data">No menu options configured.</td></tr>`;return}let r=t.menu_options.sort((e,t)=>e.digit===t.digit?0:e.digit===`*`?1:t.digit===`*`?-1:e.digit===`#`?1:t.digit===`#`?-1:e.digit.localeCompare(t.digit));n.innerHTML=``;for(let e of r){let t=document.createElement(`tr`),r=document.createElement(`td`);r.innerHTML=`<strong>${G(e.digit)}</strong>`,t.appendChild(r);let i=document.createElement(`td`);i.innerHTML=`📞 extension`,t.appendChild(i);let a=document.createElement(`td`);a.textContent=e.destination,t.appendChild(a);let o=document.createElement(`td`);o.textContent=e.description,t.appendChild(o);let s=document.createElement(`td`),c=document.createElement(`button`);c.className=`btn btn-primary`,c.textContent=`✏️ Edit`,c.onclick=()=>eo(e.digit,`extension`,e.destination,e.description),s.appendChild(c);let l=document.createElement(`button`);l.className=`btn btn-danger`,l.textContent=`🗑️ Delete`,l.onclick=()=>no(e.digit),s.appendChild(l),t.appendChild(s),n.appendChild(t)}}catch(e){throw console.error(`Error loading legacy menu options:`,e),e}}function Ya(e){return{extension:`📞`,submenu:`📁`,queue:`👥`,voicemail:`📧`,operator:`🎧`}[e]??`❓`}async function Xa(e){K=e,await W()}async function Za(){try{let e=await fetch(`${API_BASE}/api/auto-attendant/menus/${K}`,{headers:pbxAuthHeaders()});if(e.ok){let t=await e.json(),n=document.getElementById(`breadcrumb-path`);if(n){let e=G(t.menu.menu_name);K!==`main`&&(e=`<button class="btn btn-secondary" onclick="navigateToMenu('main')" style="margin-right: 10px;">⬅️ Back to Main</button> ${e}`),n.innerHTML=e}}}catch(e){console.error(`Error updating breadcrumb:`,e)}}document.addEventListener(`DOMContentLoaded`,function(){let e=document.getElementById(`auto-attendant-config-form`);e&&e.addEventListener(`submit`,async function(e){e.preventDefault();let t={enabled:document.getElementById(`aa-enabled`).checked,extension:document.getElementById(`aa-extension`).value,timeout:parseInt(document.getElementById(`aa-timeout`).value,10),max_retries:parseInt(document.getElementById(`aa-max-retries`).value,10)};try{let e=await fetch(`${API_BASE}/api/auto-attendant/config`,{method:`PUT`,headers:pbxAuthHeaders(),body:JSON.stringify(t)});if(e.ok)showNotification(`Auto attendant configuration saved successfully`,`success`);else{let t=await e.json();showNotification(t.error||`Failed to save configuration`,`error`)}}catch(e){console.error(`Error saving auto attendant config:`,e),showNotification(`Failed to save configuration`,`error`)}})});function Qa(){document.getElementById(`add-menu-option-modal`).classList.add(`active`),document.getElementById(`add-menu-option-form`).reset()}function $a(){document.getElementById(`add-menu-option-modal`).classList.remove(`active`)}function eo(e,t,n,r){document.getElementById(`edit-menu-digit`).value=e,document.getElementById(`edit-menu-digit-display`).textContent=e,document.getElementById(`edit-menu-dest-type`).value=t??`extension`,document.getElementById(`edit-menu-description`).value=r,so(`edit`),t===`submenu`?document.getElementById(`edit-menu-submenu`).value=n:document.getElementById(`edit-menu-destination`).value=n,document.getElementById(`edit-menu-option-modal`).classList.add(`active`)}function to(){document.getElementById(`edit-menu-option-modal`).classList.remove(`active`)}async function no(e){if(confirm(`Are you sure you want to delete menu option for digit "${e}"?`))try{let t=await fetch(`${API_BASE}/api/auto-attendant/menus/${K}/items/${e}`,{method:`DELETE`,headers:pbxAuthHeaders()});if(t.ok)showNotification(`Menu option deleted successfully`,`success`),W();else{let e=await t.json();showNotification(e.error||`Failed to delete menu option`,`error`)}}catch(e){console.error(`Error deleting menu option:`,e),showNotification(`Failed to delete menu option`,`error`)}}document.addEventListener(`DOMContentLoaded`,function(){let e=document.getElementById(`add-menu-option-form`);e&&e.addEventListener(`submit`,async function(e){e.preventDefault();let t=document.getElementById(`new-menu-dest-type`).value,n;n=t===`submenu`?document.getElementById(`new-menu-submenu`).value:document.getElementById(`new-menu-destination`).value;let r={digit:document.getElementById(`new-menu-digit`).value,destination_type:t,destination_value:n,description:document.getElementById(`new-menu-description`).value};try{let e=await fetch(`${API_BASE}/api/auto-attendant/menus/${K}/items`,{method:`POST`,headers:pbxAuthHeaders(),body:JSON.stringify(r)});if(e.ok)showNotification(`Menu option added successfully`,`success`),$a(),W();else{let t=await e.json();showNotification(t.error||`Failed to add menu option`,`error`)}}catch(e){console.error(`Error adding menu option:`,e),showNotification(`Failed to add menu option`,`error`)}})}),document.addEventListener(`DOMContentLoaded`,function(){let e=document.getElementById(`edit-menu-option-form`);e&&e.addEventListener(`submit`,async function(e){e.preventDefault();let t=document.getElementById(`edit-menu-digit`).value,n=document.getElementById(`edit-menu-dest-type`).value,r;r=n===`submenu`?document.getElementById(`edit-menu-submenu`).value:document.getElementById(`edit-menu-destination`).value;let i={destination_type:n,destination_value:r,description:document.getElementById(`edit-menu-description`).value};try{let e=await fetch(`${API_BASE}/api/auto-attendant/menus/${K}/items/${t}`,{method:`PUT`,headers:pbxAuthHeaders(),body:JSON.stringify(i)});if(e.ok)showNotification(`Menu option updated successfully`,`success`),to(),W();else{let t=await e.json();showNotification(t.error||`Failed to update menu option`,`error`)}}catch(e){console.error(`Error updating menu option:`,e),showNotification(`Failed to update menu option`,`error`)}})}),document.addEventListener(`DOMContentLoaded`,function(){let e=document.getElementById(`auto-attendant-prompts-form`);e&&e.addEventListener(`submit`,async function(e){e.preventDefault();let t=document.getElementById(`voice-generation-status`),n=document.getElementById(`voice-generation-message`);t&&(t.style.display=`block`,n.textContent=`⏳ Saving prompts and regenerating voices using gTTS...`);let r={company_name:document.getElementById(`aa-company-name`).value,prompts:{welcome:document.getElementById(`aa-prompt-welcome`).value,main_menu:document.getElementById(`aa-prompt-main-menu`).value,invalid:document.getElementById(`aa-prompt-invalid`).value,timeout:document.getElementById(`aa-prompt-timeout`).value,transferring:document.getElementById(`aa-prompt-transferring`).value}};try{let e=await fetch(`${API_BASE}/api/auto-attendant/prompts`,{method:`PUT`,headers:pbxAuthHeaders(),body:JSON.stringify(r)});if(e.ok)showNotification(`Prompts saved and voices regenerated successfully!`,`success`),t&&(n.textContent=`✅ Voice prompts regenerated successfully using gTTS!`,setTimeout(()=>{t.style.display=`none`},3e3));else{let r=await e.json();showNotification(r.error||`Failed to save prompts`,`error`),t&&(n.textContent=`❌ Failed to regenerate voices`,setTimeout(()=>{t.style.display=`none`},3e3))}}catch(e){console.error(`Error saving prompts:`,e),showNotification(`Failed to save prompts`,`error`),t&&(n.textContent=`❌ Error: `+e.message,setTimeout(()=>{t.style.display=`none`},3e3))}})});function G(e){let t=document.createElement(`div`);return t.textContent=e,t.innerHTML}var K=`main`,ro=[];async function io(e){try{return await e.json()}catch(e){let t=e?.message||String(e);return e instanceof SyntaxError||/JSON|Unexpected token/i.test(t)?{error:`Server returned an invalid response format`}:e instanceof TypeError||/network|NetworkError|fetch/i.test(t)?{error:`Network error while reading server response`}:(debugWarn(`Unexpected error parsing response:`,e),{error:`Unable to read server response`})}}async function ao(){try{let e=await fetch(`${API_BASE}/api/auto-attendant/menus`,{headers:pbxAuthHeaders()});if(e.ok)return ro=(await e.json()).menus??[],debugLog(`Loaded ${ro.length} menu(s) for submenu selection`),ro;{let t=await io(e);console.error(`Failed to load menus: ${e.status} ${e.statusText}`,t);let n=t.error||e.statusText;e.status===404?n+=`. API endpoint not found - check server version.`:e.status>=500&&(n+=`. Server error occurred.`),showNotification(`Failed to load menus: ${n}`,`error`)}}catch(e){console.error(`Error loading menus:`,e),showNotification(`Unable to connect to API: ${e.message}. Please check your connection.`,`error`)}return[]}async function oo(){let e=await ao(),t=document.getElementById(`new-menu-submenu`);if(t)if(t.innerHTML=`<option value="">Select a submenu</option>`,e.length===0){let e=document.createElement(`option`);e.value=``,e.textContent=`No menus available - create one first`,e.disabled=!0,t.appendChild(e),t.disabled=!0}else{for(let n of e)if(n.menu_id!==K){let e=document.createElement(`option`);e.value=n.menu_id,e.textContent=`${n.menu_name} (${n.menu_id})`,t.appendChild(e)}t.disabled=!1}let n=document.getElementById(`edit-menu-submenu`);if(n)if(n.innerHTML=`<option value="">Select a submenu</option>`,e.length===0){let e=document.createElement(`option`);e.value=``,e.textContent=`No menus available - create one first`,e.disabled=!0,n.appendChild(e),n.disabled=!0}else{for(let t of e)if(t.menu_id!==K){let e=document.createElement(`option`);e.value=t.menu_id,e.textContent=`${t.menu_name} (${t.menu_id})`,n.appendChild(e)}n.disabled=!1}let r=document.getElementById(`submenu-parent`);if(r)if(r.innerHTML=``,e.length===0){let e=document.createElement(`option`);e.value=``,e.textContent=`No parent menus available - check API connection or server status`,e.disabled=!0,e.selected=!0,r.appendChild(e),debugWarn(`No menus loaded for parent dropdown`)}else{for(let t of e){let e=document.createElement(`option`);e.value=t.menu_id,e.textContent=`${t.menu_name} (${t.menu_id})`,t.menu_id===`main`&&(e.selected=!0),r.appendChild(e)}debugLog(`Populated parent menu dropdown with ${e.length} menu(s)`)}}function so(e){let t=document.getElementById(`${e}-menu-dest-type`),n=document.getElementById(`${e}-dest-extension-group`),r=document.getElementById(`${e}-dest-submenu-group`),i=document.getElementById(`${e}-menu-destination`),a=document.getElementById(`${e}-menu-submenu`),o=document.getElementById(`${e}-dest-label`),s=document.getElementById(`${e}-dest-help`);if(!t)return;let c=t.value;if(c===`submenu`)n&&(n.style.display=`none`),r&&(r.style.display=`block`),i&&(i.required=!1),a&&(a.required=!0),oo();else if(n&&(n.style.display=`block`),r&&(r.style.display=`none`),i&&(i.required=!0),a&&(a.required=!1),o)switch(c){case`extension`:o.textContent=`Extension`,s&&(s.textContent=`Extension number to transfer to`);break;case`queue`:o.textContent=`Queue`,s&&(s.textContent=`Queue extension number`);break;case`voicemail`:o.textContent=`Voicemail Box`,s&&(s.textContent=`Voicemail box extension`);break;case`operator`:o.textContent=`Operator Extension`,s&&(s.textContent=`Operator extension number`);break}}function co(){oo(),document.getElementById(`create-submenu-modal`).classList.add(`active`),document.getElementById(`create-submenu-form`).reset()}function lo(){document.getElementById(`create-submenu-modal`).classList.remove(`active`)}async function uo(){let e=document.getElementById(`menu-tree-container`);e.style.display===`none`?(e.style.display=`block`,await fo()):e.style.display=`none`}async function fo(){try{let e=await fetch(`${API_BASE}/api/auto-attendant/menu-tree`,{headers:pbxAuthHeaders()});if(!e.ok){let t=await io(e);throw console.error(`Failed to load menu tree: ${e.status} ${e.statusText}`,t),e.status===404?Error(`Menu tree endpoint not found. The server may need to be restarted to load new API routes.`):e.status===500?Error(`Server error: ${t.error||`Internal server error`}`):Error(`Failed to load menu tree: ${t.error||e.statusText}`)}let t=await e.json(),n=document.getElementById(`menu-tree-view`);t.menu_tree?(n.innerHTML=po(t.menu_tree,0),debugLog(`Menu tree loaded successfully`)):(n.innerHTML=`<p style="color: #666;">No menu structure available</p>`,debugWarn(`Menu tree data is empty`))}catch(e){console.error(`Error loading menu tree:`,e),showNotification(`Failed to load menu tree: ${e.message}`,`error`);let t=document.getElementById(`menu-tree-view`);t&&(t.innerHTML=`<p class="error-message">
    ...
    `,!document.getElementById(`notification-animations`)){let e=document.createElement(`style`);e.id=`notification-animations`,e.textContent=`
        ...
        `,document.head.appendChild(e)}r.appendChild(i),n>0&&setTimeout(()=>{i.style.animation=`slideIn 0.3s ease-out reverse`,setTimeout(()=>i.remove(),300)},n)}async function go(){bo(),Co(),Eo(),await Y()}async function Y(){try{let e=(await(await fetch(`/api/config`,{headers:pbxAuthHeaders()})).json()).integrations||{},t=e.jitsi?.enabled||!1,n=document.getElementById(`quick-jitsi-enabled`),r=document.getElementById(`jitsi-status-badge`);n&&(n.checked=t),r&&(r.style.display=t?`inline-block`:`none`,r.style.backgroundColor=`#4CAF50`,r.style.color=`white`,r.textContent=`● Enabled`);let i=e.matrix?.enabled||!1,a=document.getElementById(`quick-matrix-enabled`),o=document.getElementById(`matrix-status-badge`);a&&(a.checked=i),o&&(o.style.display=i?`inline-block`:`none`,o.style.backgroundColor=`#9C27B0`,o.style.color=`white`,o.textContent=`● Enabled`);let s=e.espocrm?.enabled||!1,c=document.getElementById(`quick-espocrm-enabled`),l=document.getElementById(`espocrm-status-badge`);c&&(c.checked=s),l&&(l.style.display=s?`inline-block`:`none`,l.style.backgroundColor=`#2196F3`,l.style.color=`white`,l.textContent=`● Enabled`)}catch(e){console.error(`Failed to load integration status:`,e)}}async function _o(e){document.getElementById(`quick-${e}-enabled`).checked?await vo(e):await yo(e)}async function vo(e){let t={jitsi:{enabled:!0,server_url:`https://localhost`,auto_create_rooms:!0,app_id:``,app_secret:``},matrix:{enabled:!0,homeserver_url:`https://localhost:8008`,bot_username:``,bot_password:"${MATRIX_BOT_PASSWORD}",notification_room:``,voicemail_room:``,missed_call_notifications:!0},espocrm:{enabled:!0,api_url:`https://localhost/api/v1`,api_key:"${ESPOCRM_API_KEY}",auto_create_contacts:!0,auto_log_calls:!0,screen_pop:!0}}[e];if(!t){J(`Unknown integration: ${e}`,`error`);return}try{if((await fetch(`/api/config/section`,{method:`PUT`,headers:pbxAuthHeaders(),body:JSON.stringify({section:`integrations`,data:{[e]:t}})})).ok){let t=document.getElementById(`quick-${e}-enabled`);t&&(t.checked=!0),Y();let n=`✅ ${ho[e]} enabled with default settings! The integration is now active.`;e===`matrix`?J(`${n} Note: You need to set MATRIX_BOT_PASSWORD in your .env file for Matrix to work.`,`warning`,8e3):e===`espocrm`?J(`${n} Note: You need to set ESPOCRM_API_KEY and api_url in the configuration tab.`,`warning`,8e3):J(n,`success`),e===`jitsi`?bo():e===`matrix`?Co():e===`espocrm`&&Eo()}else{J(`Failed to enable ${ho[e]}`,`error`);let t=document.getElementById(`quick-${e}-enabled`);t&&(t.checked=!1)}}catch(t){J(`Error enabling integration: ${t.message}`,`error`);let n=document.getElementById(`quick-${e}-enabled`);n&&(n.checked=!1)}}async function yo(e){try{let t=(await(await fetch(`/api/config`,{headers:pbxAuthHeaders()})).json()).integrations?.[e]??{};if(t.enabled=!1,(await fetch(`/api/config/section`,{method:`PUT`,headers:pbxAuthHeaders(),body:JSON.stringify({section:`integrations`,data:{[e]:t}})})).ok)Y(),J(`${ho[e]} has been disabled.`,`info`),e===`jitsi`?bo():e===`matrix`?Co():e===`espocrm`&&Eo();else{J(`Failed to disable ${ho[e]}`,`error`);let t=document.getElementById(`quick-${e}-enabled`);t&&(t.checked=!0)}}catch(t){J(`Error disabling integration: ${t.message}`,`error`);let n=document.getElementById(`quick-${e}-enabled`);n&&(n.checked=!0)}}async function bo(){try{let e=await fetch(`/api/config`,{headers:pbxAuthHeaders()});if(!e.ok){window.suppressErrorNotifications?debugLog(`Config endpoint returned error:`,e.status,`(feature may not be available or authentication required)`):console.error(`Failed to load Jitsi config:`,e.status);return}let t=(await e.json()).integrations?.jitsi??{};document.getElementById(`jitsi-enabled`).checked=t.enabled??!1,document.getElementById(`jitsi-server-url`).value=t.server_url??`https://localhost`,document.getElementById(`jitsi-auto-create-rooms`).checked=t.auto_create_rooms!==!1,document.getElementById(`jitsi-app-id`).value=t.app_id??``,document.getElementById(`jitsi-app-secret`).value=t.app_secret??``,xo()}catch(e){window.suppressErrorNotifications?debugLog(`Failed to load Jitsi config (expected if not authenticated):`,e.message):console.error(`Failed to load Jitsi config:`,e)}}function xo(){let e=document.getElementById(`jitsi-enabled`).checked;document.getElementById(`jitsi-settings`).style.display=e?`block`:`none`}document.getElementById(`jitsi-enabled`)?.addEventListener(`change`,xo),document.getElementById(`jitsi-config-form`)?.addEventListener(`submit`,async function(e){e.preventDefault();let t={enabled:document.getElementById(`jitsi-enabled`).checked,server_url:document.getElementById(`jitsi-server-url`).value,auto_create_rooms:document.getElementById(`jitsi-auto-create-rooms`).checked,app_id:document.getElementById(`jitsi-app-id`).value,app_secret:document.getElementById(`jitsi-app-secret`).value};try{(await fetch(`/api/config/section`,{method:`PUT`,headers:pbxAuthHeaders(),body:JSON.stringify({section:`integrations`,data:{jitsi:t}})})).ok?(X(`Configuration saved successfully!`,`success`),Y()):X(`Failed to save configuration`,`error`)}catch(e){X(`Error: ${q(e.message)}`,`error`)}});async function So(){let e=document.getElementById(`jitsi-server-url`).value;X(`Testing connection to ${q(e)}...`,`info`);try{let t=`${e}/external_api.js`;await fetch(t,{mode:`no-cors`});let n=`${e}/test-pbx-${Date.now()}`;X(`✅ Connection successful!<br>Test meeting URL: <a href="${q(n)}" target="_blank">${q(n)}</a>`,`success`)}catch(e){X(`⚠️ Could not verify connection. Server may still be accessible.<br>Error: ${q(e.message)}`,`warning`)}}function X(e,t){let n=[`success`,`error`,`warning`,`info`].includes(t)?t:`info`,r=document.getElementById(`jitsi-status`);r.innerHTML=`<div class="alert alert-${n}">${e}</div>`}async function Co(){try{let e=await fetch(`/api/config`,{headers:pbxAuthHeaders()});if(!e.ok){window.suppressErrorNotifications?debugLog(`Config endpoint returned error:`,e.status,`(feature may not be available or authentication required)`):console.error(`Failed to load Matrix config:`,e.status);return}let t=(await e.json()).integrations?.matrix??{};document.getElementById(`matrix-enabled`).checked=t.enabled??!1,document.getElementById(`matrix-homeserver-url`).value=t.homeserver_url??`https://localhost:8008`,document.getElementById(`matrix-bot-username`).value=t.bot_username??``,document.getElementById(`matrix-bot-password`).value=t.bot_password??``,document.getElementById(`matrix-notification-room`).value=t.notification_room??``,document.getElementById(`matrix-voicemail-room`).value=t.voicemail_room??``,document.getElementById(`matrix-missed-call-notifications`).checked=t.missed_call_notifications!==!1,wo()}catch(e){window.suppressErrorNotifications?debugLog(`Failed to load Matrix config (expected if not authenticated):`,e.message):console.error(`Failed to load Matrix config:`,e)}}function wo(){let e=document.getElementById(`matrix-enabled`).checked;document.getElementById(`matrix-settings`).style.display=e?`block`:`none`}document.getElementById(`matrix-enabled`)?.addEventListener(`change`,wo),document.getElementById(`matrix-config-form`)?.addEventListener(`submit`,async function(e){e.preventDefault();let t={enabled:document.getElementById(`matrix-enabled`).checked,homeserver_url:document.getElementById(`matrix-homeserver-url`).value,bot_username:document.getElementById(`matrix-bot-username`).value,bot_password:document.getElementById(`matrix-bot-password`).value,notification_room:document.getElementById(`matrix-notification-room`).value,voicemail_room:document.getElementById(`matrix-voicemail-room`).value,missed_call_notifications:document.getElementById(`matrix-missed-call-notifications`).checked};try{(await fetch(`/api/config/section`,{method:`PUT`,headers:pbxAuthHeaders(),body:JSON.stringify({section:`integrations`,data:{matrix:t}})})).ok?(Z(`Configuration saved successfully!`,`success`),Y()):Z(`Failed to save configuration`,`error`)}catch(e){Z(`Error: ${q(e.message)}`,`error`)}});async function To(){let e=document.getElementById(`matrix-homeserver-url`).value,t=document.getElementById(`matrix-bot-username`).value,n=document.getElementById(`matrix-bot-password`).value;if(!t||!n){Z(`Please enter bot username and password`,`error`);return}Z(`Testing Matrix connection...`,`info`);try{let t=`${e}/_matrix/client/versions`,n=await fetch(t);if(!n.ok)throw Error(`Homeserver not accessible`);Z(`✅ Homeserver is accessible!<br>Supported versions: ${q((await n.json()).versions?.join(`, `)??`Unknown`)}<br><small>Note: Full authentication test requires server-side validation</small>`,`success`)}catch(e){Z(`❌ Connection failed: ${q(e.message)}`,`error`)}}function Z(e,t){let n=[`success`,`error`,`warning`,`info`].includes(t)?t:`info`,r=document.getElementById(`matrix-status`);r.innerHTML=`<div class="alert alert-${n}">${e}</div>`}async function Eo(){try{let e=await fetch(`/api/config`,{headers:pbxAuthHeaders()});if(!e.ok){window.suppressErrorNotifications?debugLog(`Config endpoint returned error:`,e.status,`(feature may not be available or authentication required)`):console.error(`Failed to load EspoCRM config:`,e.status);return}let t=(await e.json()).integrations?.espocrm??{};document.getElementById(`espocrm-enabled`).checked=t.enabled??!1,document.getElementById(`espocrm-api-url`).value=t.api_url??`https://localhost/api/v1`,document.getElementById(`espocrm-api-key`).value=t.api_key??``,document.getElementById(`espocrm-auto-create-contacts`).checked=t.auto_create_contacts!==!1,document.getElementById(`espocrm-auto-log-calls`).checked=t.auto_log_calls!==!1,document.getElementById(`espocrm-screen-pop`).checked=t.screen_pop!==!1,Do()}catch(e){window.suppressErrorNotifications?debugLog(`Failed to load EspoCRM config (expected if not authenticated):`,e.message):console.error(`Failed to load EspoCRM config:`,e)}}function Do(){let e=document.getElementById(`espocrm-enabled`).checked;document.getElementById(`espocrm-settings`).style.display=e?`block`:`none`}document.getElementById(`espocrm-enabled`)?.addEventListener(`change`,Do),document.getElementById(`espocrm-config-form`)?.addEventListener(`submit`,async function(e){e.preventDefault();let t={enabled:document.getElementById(`espocrm-enabled`).checked,api_url:document.getElementById(`espocrm-api-url`).value,api_key:document.getElementById(`espocrm-api-key`).value,auto_create_contacts:document.getElementById(`espocrm-auto-create-contacts`).checked,auto_log_calls:document.getElementById(`espocrm-auto-log-calls`).checked,screen_pop:document.getElementById(`espocrm-screen-pop`).checked};try{(await fetch(`/api/config/section`,{method:`PUT`,headers:pbxAuthHeaders(),body:JSON.stringify({section:`integrations`,data:{espocrm:t}})})).ok?(Q(`Configuration saved successfully!`,`success`),Y()):Q(`Failed to save configuration`,`error`)}catch(e){Q(`Error: ${q(e.message)}`,`error`)}});async function Oo(){let e=document.getElementById(`espocrm-api-url`).value,t=document.getElementById(`espocrm-api-key`).value;if(!e||!t){Q(`Please enter API URL and API Key`,`error`);return}Q(`Testing EspoCRM connection...`,`info`);try{let n=`${e}/App/user`,r=await fetch(n,{headers:{"X-Api-Key":t,"Content-Type":`application/json`}});if(r.ok)Q(`✅ Connection successful!<br>Connected as: ${q((await r.json()).userName??`Unknown`)}<br>EspoCRM is ready for integration.`,`success`);else throw Error(`API returned status ${r.status}`)}catch(e){Q(`❌ Connection failed: ${q(e.message)}<br>Check API URL and API Key.`,`error`)}}function Q(e,t){let n=[`success`,`error`,`warning`,`info`].includes(t)?t:`info`,r=document.getElementById(`espocrm-status`);r.innerHTML=`<div class="alert alert-${n}">${e}</div>`}async function ko(){let e=document.getElementById(`jitsi-instant-room`).value??``;try{let t=await fetch(`/api/integrations/jitsi/instant`,{method:`POST`,headers:pbxAuthHeaders(),body:JSON.stringify({room_name:e,extension:`admin`})});if(!t.ok){let e=await t.json();throw Error(e.error||`Failed to create meeting`)}jo((await t.json()).meeting_url),J(`Meeting created successfully!`,`success`)}catch(e){J(`Failed to create meeting: ${e.message}`,`error`)}}async function Ao(){let e=document.getElementById(`jitsi-schedule-subject`).value,t=document.getElementById(`jitsi-schedule-duration`).value;if(!e){J(`Please enter a meeting subject`,`warning`);return}try{let n=await fetch(`/api/integrations/jitsi/meetings`,{method:`POST`,headers:pbxAuthHeaders(),body:JSON.stringify({subject:e,duration:parseInt(t),moderator_name:`Admin`})});if(!n.ok){let e=await n.json();throw Error(e.error||`Failed to schedule meeting`)}jo((await n.json()).meeting_url),J(`Meeting scheduled successfully!`,`success`)}catch(e){J(`Failed to schedule meeting: ${e.message}`,`error`)}}function jo(e){let t=document.getElementById(`jitsi-meeting-result`),n=document.getElementById(`jitsi-meeting-url`);n.value=e,t.style.display=`block`}async function Mo(){let e=document.getElementById(`jitsi-meeting-url`),t=e.value;try{navigator.clipboard&&navigator.clipboard.writeText?(await navigator.clipboard.writeText(t),J(`Meeting URL copied to clipboard!`,`success`,3e3)):(e.select(),document.execCommand(`copy`),J(`Meeting URL copied to clipboard!`,`success`,3e3))}catch{e.select(),document.execCommand(`copy`),J(`Meeting URL copied to clipboard!`,`success`,3e3)}}function No(){let e=document.getElementById(`jitsi-meeting-url`).value;e&&window.open(e,`_blank`)}document.addEventListener(`DOMContentLoaded`,function(){let e=document.getElementById(`matrix-room-select`);e&&e.addEventListener(`change`,function(){let e=document.getElementById(`matrix-custom-room`);this.value===`custom`?e.style.display=`block`:e.style.display=`none`})});async function Po(){let e=document.getElementById(`matrix-room-select`).value,t=document.getElementById(`matrix-custom-room-id`).value,n=document.getElementById(`matrix-message-text`).value;if(!n){J(`Please enter a message`,`warning`);return}let r=null;if(e===`custom`){if(r=t,!r){J(`Please enter a custom room ID`,`warning`);return}}else (e===`notification`||e===`voicemail`)&&(r=null);try{let e=await fetch(`/api/integrations/matrix/messages`,{method:`POST`,headers:pbxAuthHeaders(),body:JSON.stringify({room_id:r,message:n,msg_type:`m.text`})});if(!e.ok){let t=await e.json();throw Error(t.error||`Failed to send message`)}await e.json(),$(`✅ Message sent successfully!`,`success`),document.getElementById(`matrix-message-text`).value=``}catch(e){$(`❌ Failed to send message: ${e.message}`,`error`)}}async function Fo(){try{let e=await fetch(`/api/integrations/matrix/notifications`,{method:`POST`,headers:pbxAuthHeaders(),body:JSON.stringify({message:`🧪 Test notification from PBX Admin Panel - ${new Date().toLocaleString()}`})});if(!e.ok){let t=await e.json();throw Error(t.error||`Failed to send notification`)}$(`✅ Test notification sent successfully!`,`success`)}catch(e){$(`❌ Failed to send notification: ${e.message}`,`error`)}}async function Io(){let e=document.getElementById(`matrix-new-room-name`).value,t=document.getElementById(`matrix-new-room-topic`).value;if(!e){J(`Please enter a room name`,`warning`);return}try{let n=await fetch(`/api/integrations/matrix/rooms`,{method:`POST`,headers:pbxAuthHeaders(),body:JSON.stringify({name:e,topic:t})});if(!n.ok){let e=await n.json();throw Error(e.error||`Failed to create room`)}Lo((await n.json()).room_id),J(`Room created successfully!`,`success`),document.getElementById(`matrix-new-room-name`).value=``,document.getElementById(`matrix-new-room-topic`).value=``}catch(e){J(`Failed to create room: ${e.message}`,`error`)}}function $(e,t){let n=document.getElementById(`matrix-message-result`),r=document.getElementById(`matrix-message-result-text`);r.textContent=e,n.style.display=`block`,n.querySelector(`.info-box`).style.backgroundColor=t===`success`?`#e8f5e9`:`#ffebee`,setTimeout(()=>{n.style.display=`none`},5e3)}function Lo(e){let t=document.getElementById(`matrix-room-result`),n=document.getElementById(`matrix-new-room-id`);n.textContent=e,t.style.display=`block`}async function Ro(){let e=document.getElementById(`espocrm-search-type`).value,t=document.getElementById(`espocrm-search-term`).value;if(!t){J(`Please enter a search term`,`warning`);return}try{let n=await fetch(`/api/integrations/espocrm/contacts/search?${e===`phone`?`phone`:e===`email`?`email`:`name`}=${encodeURIComponent(t)}`,{method:`GET`,headers:pbxAuthHeaders()});if(!n.ok){let e=await n.json();throw Error(e.error||`Failed to search contact`)}zo(await n.json())}catch(e){J(`Failed to search contact: ${e.message}`,`error`)}}function zo(e){let t=document.getElementById(`espocrm-search-results`),n=document.getElementById(`espocrm-contact-details`);if(!e.success||!e.contact){n.innerHTML=`<div class="info-box" style="background-color: #fff3e0;">No contact found</div>`,t.style.display=`block`;return}let r=e.contact,i=`<div class="info-box" style="background-color: #e8f5e9;">`;i+=`<h4>✅ Contact Found</h4>`,i+=`<table style="width: 100%; margin-top: 10px;">`,r.name&&(i+=`<tr><td><strong>Name:</strong></td><td>${q(r.name)}</td></tr>`),r.email&&(i+=`<tr><td><strong>Email:</strong></td><td>${q(r.email)}</td></tr>`),r.phone&&(i+=`<tr><td><strong>Phone:</strong></td><td>${q(r.phone)}</td></tr>`),r.company&&(i+=`<tr><td><strong>Company:</strong></td><td>${q(r.company)}</td></tr>`),r.title&&(i+=`<tr><td><strong>Title:</strong></td><td>${q(r.title)}</td></tr>`),r.id&&(i+=`<tr><td><strong>CRM ID:</strong></td><td>${q(r.id)}</td></tr>`),i+=`</table></div>`,n.innerHTML=i,t.style.display=`block`}async function Bo(){let e=document.getElementById(`espocrm-new-firstname`).value,t=document.getElementById(`espocrm-new-lastname`).value,n=document.getElementById(`espocrm-new-phone`).value,r=document.getElementById(`espocrm-new-email`).value,i=document.getElementById(`espocrm-new-company`).value,a=document.getElementById(`espocrm-new-title`).value;if(!e||!t){J(`Please enter first and last name`,`warning`);return}if(!n&&!r){J(`Please enter at least phone or email`,`warning`);return}try{let o=await fetch(`/api/integrations/espocrm/contacts`,{method:`POST`,headers:pbxAuthHeaders(),body:JSON.stringify({name:`${e} ${t}`,phone:n,email:r,company:i,title:a})});if(!o.ok){let e=await o.json();throw Error(e.error||`Failed to create contact`)}Vo(`✅ Contact created successfully! CRM ID: ${(await o.json()).contact?.id??`unknown`}`),document.getElementById(`espocrm-new-firstname`).value=``,document.getElementById(`espocrm-new-lastname`).value=``,document.getElementById(`espocrm-new-phone`).value=``,document.getElementById(`espocrm-new-email`).value=``,document.getElementById(`espocrm-new-company`).value=``,document.getElementById(`espocrm-new-title`).value=``}catch(e){Vo(`❌ Failed to create contact: ${e.message}`)}}function Vo(e){let t=document.getElementById(`espocrm-create-result`),n=document.getElementById(`espocrm-create-result-text`);n.textContent=e,t.style.display=`block`,setTimeout(()=>{t.style.display=`none`},5e3)}window.loadOpenSourceIntegrations=go,window.loadJitsiConfig=bo,window.loadMatrixConfig=Co,window.loadEspoCRMConfig=Eo,window.toggleJitsiSettings=xo,window.testJitsiConnection=So,window.toggleMatrixSettings=wo,window.testMatrixConnection=To,window.toggleEspoCRMSettings=Do,window.testEspoCRMConnection=Oo,window.quickToggleIntegration=_o,window.quickSetupIntegration=vo,window.disableIntegration=yo,window.createInstantJitsiMeeting=ko,window.scheduleJitsiMeeting=Ao,window.copyJitsiMeetingUrl=Mo,window.openJitsiMeeting=No,window.sendMatrixMessage=Po,window.sendMatrixTestNotification=Fo,window.createMatrixRoom=Io,window.searchEspoCRMContact=Ro,window.createEspoCRMContact=Bo,setTimeout(function(){if(!window.currentUser&&!document.querySelector(`.tab-content.active`)){console.warn(`Page may not have loaded correctly. Checking for common issues...`);var e=document.querySelector(`.sidebar`);if(e){var t=window.getComputedStyle(e);if(t.width===`auto`||t.width===`0px`){console.error(`CSS may not be loaded correctly. Try clearing your browser cache:`),console.error(`  - Press Ctrl+Shift+R (Windows/Linux) or Cmd+Shift+R (Mac)`),console.error(`  - See BROWSER_CACHE_FIX.md for detailed instructions`);var n=document.createElement(`div`);n.className=`page-load-alert`,n.innerHTML=`<strong class="page-load-alert-title">⚠️ Page Loading Issue</strong><p>The admin panel may not be displaying correctly due to cached files.</p><p class="page-load-alert-action">Press <code>Ctrl+Shift+R</code> (or <code>Cmd+Shift+R</code> on Mac) to reload without cache</p>`;var r=document.createElement(`button`);r.className=`page-load-alert-dismiss`,r.textContent=`Dismiss`,r.addEventListener(`click`,function(){n.remove()}),n.appendChild(r),document.body.appendChild(n)}}}else console.log(`Page loaded successfully at`,new Date().toISOString())},3e3),(function(){var e=document.getElementById(`sidebar-toggle`),t=document.querySelector(`.sidebar`),n=document.getElementById(`sidebar-overlay`);if(!e||!t||!n)return;function r(){t.classList.add(`open`),n.classList.add(`active`),e.classList.add(`active`),e.setAttribute(`aria-expanded`,`true`)}function i(){t.classList.remove(`open`),n.classList.remove(`active`),e.classList.remove(`active`),e.setAttribute(`aria-expanded`,`false`)}e.addEventListener(`click`,function(){t.classList.contains(`open`)?i():r()}),n.addEventListener(`click`,i),t.addEventListener(`click`,function(e){e.target.closest(`.tab-button`)&&i()}),document.addEventListener(`keydown`,function(e){e.key===`Escape`&&t.classList.contains(`open`)&&i()})})();