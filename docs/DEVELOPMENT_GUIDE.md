...
| **E — External services** | SMTP account; FCM/APNs keys; LDAP/AD server; Vosk model file (install with `scripts/install_vosk_model.py`); CRM/Jitsi/Matrix/Zoom/Outlook credentials; a webhook receiver | Email notify, transcription, push, AD sync, screen pop, integrations, webhooks, SSO |
