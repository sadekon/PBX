...
| **C6** | Analytics tap — live audio/transcript/QoS feed into analysis engines | 🔶 recordings + RTCP QoS data exist; transcripts now persist to `call_transcripts` via `pbx/speech/store.py` (migration 1017), and the schema is deliberately shaped for live use — nullable `media_path`/durations, non-unique `call_id`. Still missing the RTP audio tap, which is the real blocker for a live feed | `features/call_recording.py`, `rtp/rtcp_monitor.py`, `speech/store.py` | Speech analytics, voice biometrics, call tagging, quality prediction, recording analytics, conversational AI |
