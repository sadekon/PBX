...
| Paging | `paging.py` + `core/paging_handler.py` | `features.paging` | 🔶 | Admin surface completed: API response contract unified (every collection wrapped under a stable key in both enabled and disabled states), `GET /api/paging/status` added so the UI shows a real disabled state, `remove_dac_device()` implemented (DELETE was a permanent 501), prompt-dialog forms replaced with modals posting the backend's own field names, and the Test Paging form wired to `POST /api/paging/test` → `PagingHandler.start_test_page()` (rings the announcer via `CallOriginator`, opens the page on answer). Page teardown now belongs solely to `PBXCore.end_call()`, so pages to a device-less zone no longer linger until the max-duration timer. Remaining: **multi-zone/all-call fan-out** — a page still reaches only the first resolved DAC device (`_resolve_dac_device()` warns when it drops the rest); needs one SIP leg + relay branch per device. Multicast paging needs real phones on a LAN segment that permits multicast. | B |
