...
| Find me / follow me | `find_me_follow_me.py` | `features.find_me_follow_me` | 🔶 | **Both ring modes implemented.** The feature module is self-contained: it keeps the config store (REST API `/api/fmfm/*`, PostgreSQL `fmfm_configs` — still created by the module rather than `utils/migrations.py`, admin UI) and now also executes the plan. `FindMeFollowMe` takes `pbx_core` from `FeatureInitializer` (without it, it stays a config store and declines every call), and consults its own `get_ring_strategy()` from `_dial_to_internal_extension()` — deliberately *before* `resolve_extension()`, so an offline desk phone follows to the mobile instead of returning 404, which was the old failure mode. The dialled extension's own phone rings first for `features.find_me_follow_me.initial_ring_time` (default 20 s, 0 disables, skipped when the config places the extension in its own list) — matching FreePBX Follow-Me's Initial Ring Time, so a config of just "my mobile" still reaches the desk first. Then rings each destination for its own `ring_time`, advancing on ring timeout, INVITE-transaction timeout, 486/603 or any final error, a 3xx from a destination whose phone forwards itself (the list is authoritative — following the redirect would re-target with the default no-answer timeout and drop the remaining destinations), or a destination that will not dial; internal destinations go through `_dial_extension_leg()`, external ones through the new `_dial_trunk_leg()` (trunk counterpart sharing `_build_trunk_invite()` with `_route_to_trunk()`, and unlike plain outbound it *does* arm a ring timer). Exhaustion always falls through to the **dialled extension's** mailbox, never the last destination's — there is no configurable no-answer destination and no overall cap, so total ring time is simply the sum of the ring times. The legacy `no_answer_destination` is dropped on read and write and gone from the admin UI; its DB column is left in place so old rows still load. Ring state lives in the feature keyed by Call-ID (nothing added to `Call`, nothing in `core/`), pruned when a new plan starts. Fixing FMFM also fixed call teardown generally: `SIPServer._handle_bye()` used to relay the hanging-up party's BYE verbatim, which names a dialog the other phone never had (on a re-targeted call the two sides do not even share a To URI), so a strict UA answered 481 and held its leg open after the PBX had ended the call. It now re-originates the BYE on the surviving party's own dialog via `_send_leg_bye()`, the helper transfer teardown already used — affects every call, not just FMFM. Guards: self-reference/caller dropped, dedupe, ring time clamped to 5 s–60 s sequentially and 5 s–`voicemail.no_answer_timeout` in a burst (there the longest leg is the caller's whole wait, so FMFM must not outlast an ordinary call), max 10 destinations, late-487 rejected by Via branch, generation counter so racing timeouts advance once. **Simultaneous mode implemented**: every destination rings at once, each on its own ring time, so a destination drops out when its time is up and the others carry on; voicemail only when the last gives up. `Call` holds one callee leg, so each burst leg is lifted off the call into an `FMFMLeg` (own Via branch, transaction, timer, trunk channel) and the first to answer is promoted back onto the call while the rest are CANCELled — after which the call is indistinguishable from a single-leg call. Responses are demultiplexed onto legs by Via branch; `PBXCore.end_call()` cancels a whole in-flight burst so a caller hanging up mid-ring stops every phone. Losing legs are CANCELled with the RFC 3326 `Reason: SIP;cause=200;text="Call completed elsewhere"` header so the phones that were merely beaten to the call do not log a missed call; a leg that genuinely rang out is cancelled without it and still logs one. The desk joins the burst rather than ringing first. Both modes dial through one `CallRouter.dial_destination()`, so extensions and trunk numbers are handled identically. The queue's deferred `ring_all` could be built on the same leg model. Ring logic covered by `tests/test_fmfm_ring_execution.py` (54 tests, no database needed); the 10 persistence tests still skip without PostgreSQL. **Admin page rebuilt on the glass treatment**: each configuration is an expandable card showing the ring plan it will actually produce — the implicit desk leg, each destination's window (`0 – 20 s`, `20 – 45 s`, …), and the voicemail fall-through — rather than a truncated comma list that hid the ordering. The page mirrors `_sanitize()` and `plan_for()` client-side to draw that, which is why `get_statistics()` now returns `initial_ring_time`: without it the UI would assume the 20 s default and misreport every plan on a deployment that tuned it. The configure dialog is rebuilt on the same idea: the editor *is* the plan, so each destination row carries the window it will occupy while its ring time is being chosen, and rows the backend will discard (duplicates, over-cap) stay visible and flagged rather than vanishing at call time. It also now opens via `.modal.active` rather than an inline `display`, which means the shared Escape handler in `ui/tabs.ts` closes it — it previously did not. A ring time out of range is now reported per row and blocks the save rather than being clamped as it is typed — the editor keeps each field's text verbatim, because holding the parsed number meant echoing it back on every keystroke, which rewrote a half-typed entry and made a two-digit value very nearly impossible to enter. `get_statistics()` therefore also returns the bounds themselves, since the simultaneous ceiling is deployment-specific. Both are pinned by `admin/tests/fmfm.test.js` (35 tests) so the rules duplicated from this module fail loudly rather than drifting. Needs field verification on Rig A, and Rig C for external destinations. | A, C |
...
| Paging | `paging.py` + `core/paging_handler.py` + `rtp/paging_media.py` | `features.paging` | 🔶 | Rebuilt on persisted zones/destinations (migration 1024). An ATA is an ordinary extension; hardware stays in `provisioned_devices`, so a dual-port ATA is two destinations off one device row. Legs go through `CallOriginator` with a vendor-derived auto-answer header; `PagingMediaSession` fans one PCMU stream out to every answered endpoint. **Untested against real hardware** — whether the Cisco ATA honours `Call-Info: answer-after=0` on an FXS port is unverified, hence the per-destination `auto_answer_override`. Multicast to desk phones is schema-ready but unimplemented; browser paging is refused. | B |
